...
A instrução explícita **“Sem novo ci backend”**, registrada anteriormente nesta sessão e não revogada pelo pedido de correção geral das pesquisas, continua aplicável a esta entrega. O usuário não repetiu a frase nesta subcorreção. A decisão é própria desta correção; a dispensa do PR918 não é reutilizada como autorização.

O escopo é a branch `codex/fix-tct-section-visibility-20260910`, originada de `02ad2d6a38301667a82eb60444b531d1e1fcd4e1`, o **PR919**, agora selado no manifesto, e sua integração exata e confirmada em main. O manifesto em bootstrap, com número nulo, bloqueia a classificação antes do fallback e não produz um gate aprovado. Abrir draft PR não suprime CI automaticamente.
