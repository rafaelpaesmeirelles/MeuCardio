...
181 verificações isoladas passaram em 0,70 s: oito valores independentes das planilhas oficiais, cada um também no formato real string do formulário; ambos os sexos e tratamentos; domínio, entradas inválidas e bloqueio de execução dos recursos externos. A revisão encontrou e corrigiu a incompatibilidade inicial entre números string da UI e o backend. TypeScript sem diagnósticos. Verificação adicional do novo registro SBC: teste focal de recursos externos passou. Verificação frontend concluída: build de produção (`npm run build`, TypeScript + Vite + PWA) passou, exit 0 em 153,27 s. O problema anterior estava no harness que importava React Router duas vezes; a verificação final usa o bundle de produção completo com App/Shell originais, sem alteração do produto. Seis casos em 360×900 (catálogo, ferramenta externa e calculadora nativa; claro/escuro) passaram sem erros JavaScript e sem overflow horizontal global. Dez verificações funcionais passaram: SBC em confirmação; ferramentas externas sem campos/cálculo e com URL oficial, `noopener noreferrer` e `no-referrer`; resultado Framingham de 7,24%; booleanos preservados; edição limpa resultado e descarta resposta assíncrona antiga. Foram interceptadas 54 chamadas de API com perfil fictício e fixtures extraídas dos registros reais, sem backend, banco ou IA paga. As capturas de catálogo em ambos os temas, ferramenta externa escura e resultado nativo escuro foram inspecionadas. Evidências duráveis: [relatório](qa/cardiovascular-risk-20260910/report.json), [catálogo claro](qa/cardiovascular-risk-20260910/catalog-risk-light.png), [ferramenta externa escura](qa/cardiovascular-risk-20260910/external-dark.png). Esta verificação não substitui teste de integração em produção. Não executar CI backend completo ou focal: instrução persistente do responsável. PR preparado para revisão, sem contornar gates obrigatórios nem declarar publicação em produção.
