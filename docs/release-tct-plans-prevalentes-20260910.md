...
O primeiro CI completo do backend do candidato `eddcb80d` terminou com **3.038 aprovados, 84 falhas e três ignorados**, em 1.821,92 segundos. Esse resultado não constitui aprovação integral. A instrução posterior e vigente do proprietário é **“Sem novo ci backend”**. Por isso, nenhum novo job de testes backend, completo ou focal, será executado para o PR #918 e sua integração exata na main. A decisão é registrada como `authorized-no-backend-ci`, sem certificado de teste e sem converter a falha anterior em aprovação. Os testes locais concluídos permanecem como evidência distinta. Os gates independentes de frontend, corpus, RC2 e publicação continuam exigidos; detalhes em `docs/ci/pr918-no-backend-ci-owner-decision.md`.
...
Por solicitação expressa do proprietário, o Heart Team passará a usar GPT-5.6 Sol dedicado em suas análises e síntese, com raciocínio `medium`, orçamento antes da execução e configuração separada do chat geral. A validação da configuração dedicada passou em 94 testes e revisão independente. O limite da extração visual foi alinhado à saída de 8.192 tokens e passou em uma conferência focal separada. O contrato final está em `heart-team-frontier-and-scoped-reconciliation-20260910.md`. As quatro variáveis dedicadas não possuem override no `.env` de produção; o novo código fornecerá os defaults aprovados sem alteração de credenciais. **Produção ainda não atualizada; correções finais de navegação e interface, gates independentes de publicação e deploy pendentes. Nenhum novo CI backend será executado.** A avaliação real do provedor semântico permanece limitada pelo `credit_balance_exhausted` já registrado; testes simulados não comprovam qualidade de respostas em produção.

## Correções finais da navegação e dos gates — 10/09/2026

O auditor agora consome a autorização schema 2 com provas, hashes e partições reais. Também detecta destinos em quarentena: a mera presença de um slug não basta para oferecer navegação. Sete equivalências bibliográficas integrais corrigem dez ocorrências de links, apontando para cinco doenças e dois estudos aprovados; o grafo preserva o tipo e registra somente `mentioned_in`.

Há **22 referências a 17 destinos indisponíveis**: dez ocorrências sem equivalente integral aprovado e 12 ocorrências para itens retidos. Na resposta de leitura, o rótulo é preservado com “conteúdo indisponível nesta versão”, sem link clicável. O registro é específico, possui prova verificável e não concede publicação. Destinos desconhecidos, prova alterada e registros obsoletos continuam bloqueando o auditor. O corpus científico e seus hashes não foram modificados. A auditoria local final contabiliza 12.210 itens, 12.154 autorizados, 56 em quarentena, zero referências quebradas navegáveis e as 22 indisponibilidades declaradas; isso não significa disponibilidade integral de todo o material.

Os quatro estágios de autorização legados e o manifesto estrito mais recente foram conferidos localmente, sem pendências. Os testes locais de disponibilidade/autorização passaram, assim como os de resolução tipada e navegação. O primeiro CI completo não foi repetido.

Os contratos visuais foram alinhados aos ativos já aprovados de `LoginGalaxy` e às dez imagens das cinco jornadas científicas. O espaçamento de desktops baixos foi reduzido para manter os controles de acesso dentro da tela. Os sete contratos científicos locais passaram; a renderização posterior será conferida pelo workflow visual existente no próximo commit.
