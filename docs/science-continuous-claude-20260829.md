...
3. `doenca-de-kawasaki` — aprofundamento (diagnostic_approach vazio, treatment_summary raso) — correção aditiva posteriormente aprovada na revisão consolidada
...
- `valvopatia-elegibilidade-esportiva-atleta` — verbete novo (maior lacuna então identificada no tema esporte, 29/54)
...
(evidência relacionada já catalogada) — já existe `content/Perioperatório/cmh-lvoto-cirurgia-nao-cardiaca-arvore-aha-acc-2024.md`;
...
- correção `doenca-de-kawasaki` — farmacoterapia de longo prazo por tamanho
  ...
  review_status confirmado revisado via load_disease_records().
...
- correção `itb-pos-exercicio-em-esteira-na-claudicacao-intermitente` — 3
  ...
  calcificação, TcPO2 complementar (WIfI ≥60/​<30mmHg). O status
  permaneceu `revisado` após a conferência final; source_refs 2→5.
...
- Correção `sarcoidose-cardiaca`: merge cirúrgico (não substituição) —
  ...
  (zzz-auditoria-703); a nova correção preservou o status após a
  conferência final. Ordenação de arquivo verificada antes do commit.
...
**Status editorial consolidado em 30/08/2026**: toda a produção nova desta
janela foi revisada e está com `review_status: revisado`. O fechamento abrange
1 caso clínico, 42 checklists, 13 exames, 13 verbetes extensos, 12 correções e
a ampliação do verbete de sarcoidose cardíaca. A regressão de status do exame
de ITB pós-exercício foi corrigida. O lote está pronto para a etapa técnica de
publicação.
