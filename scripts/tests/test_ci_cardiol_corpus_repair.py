...
with patch.object(sys, "path", [str(ROOT / "scripts"), *sys.path]):
    SPEC = importlib.util.spec_from_file_location("ci_corpus_repair_owner_policy", ROOT / "scripts/ci_backend_followup.py")
    POLICY = importlib.util.module_from_spec(SPEC)
    SPEC.loader.exec_module(POLICY)


def cardiol_decision(root, **context):
    return POLICY.scoped_owner_decision(root, scope=POLICY.CORPUS_REPAIR_SCOPE, **context)


class CorpusRepairOwnerDecisionTests(unittest.TestCase):
    def setUp(self):
        self.directory = tempfile.TemporaryDirectory()
        self.addCleanup(self.directory.cleanup)
        self.root = Path(self.directory.name)
        self.manifest = json.loads((ROOT / POLICY.CORPUS_REPAIR_SCOPE['manifest']).read_text())
        self.manifest["pull_request"] = 999
        self.write_manifest()
        for path in ["docs/cardiol-corpus-gate-repair-20260910.md", "docs/ci/cardiol-corpus-repair-owner-decision.md"]:
        ...
        self.pr = {"number": 999, "base": {"ref": "main", "repo": {"full_name": POLICY.CORPUS_REPAIR_SCOPE['repository']}},
            "head": {"ref": POLICY.CORPUS_REPAIR_SCOPE['head_branch'], "sha": self.head,
                     "repo": {"full_name": POLICY.CORPUS_REPAIR_SCOPE['repository']}},
            "merged": False, "merge_commit_sha": None}
        self.associated = []
        self.calls = []

    def write_manifest(self):
        path = self.root / POLICY.CORPUS_REPAIR_SCOPE['manifest']
    ...
    def decide(self, *, event="pull_request", number="999", head_ref=None):
        with patch.object(POLICY, "github", self.github), patch.object(POLICY, "git", self.git), \
            ...
            result = cardiol_decision(self.root, event=event, number=number,
                head_ref=POLICY.CORPUS_REPAIR_SCOPE['head_branch'] if head_ref is None else head_ref,
                repository=POLICY.CORPUS_REPAIR_SCOPE['repository'])
        ...
        try:
            with patch.object(sys, "argv", ["ci_backend_followup.py", "--repo-root", str(self.root),
                 ...
                 patch.dict(os.environ, {"EVENT_NAME": "pull_request", "PR_NUMBER": "999",
                    "PR_HEAD_REF": POLICY.CORPUS_REPAIR_SCOPE['head_branch'], "REPOSITORY": POLICY.CORPUS_REPAIR_SCOPE['repository']}), \
    ...
    def test_sealed_pr_records_owner_decision_without_tests_or_certificate(self):
        ...
        self.assertNotIn("section-visibility", result.suite_key)
        self.assertNotIn("editorial-classifications", result.suite_key)
        self.assertNotIn("feature-discovery", result.suite_key)
        self.assertNotIn("universal-favorites", result.suite_key)
    ...
    def test_api_failure_propagates_without_selecting_suite(self):
        with patch.object(POLICY, "git", self.git), \
            ...
            with self.assertRaises(subprocess.CalledProcessError):
                cardiol_decision(self.root, event="pull_request", number="999",
                    head_ref=POLICY.CORPUS_REPAIR_SCOPE['head_branch'], repository=POLICY.CORPUS_REPAIR_SCOPE['repository'])
    ...
    def test_identity_origin_and_missing_authorization_fail_closed(self):
        for field, value in [("base_sha", "0" * 40), ("repository", "other/repo"),
                             ("head_branch", "other-branch"), ("pull_request", 918), ("pull_request", 919), ("pull_request", 920), ("pull_request", 921), ("pull_request", 922)]:
    ...
    def test_wrong_head_repo_or_stale_head_never_authorizes(self):
        ...
        with self.assertRaises(ValueError): self.decide()
        self.pr["head"]["repo"]["full_name"] = POLICY.CORPUS_REPAIR_SCOPE['repository']
        self.candidate = "b" * 40
        with self.assertRaises(ValueError): self.decide()
    ...
    def test_workflow_skips_both_test_jobs_and_keeps_explicit_gate_label(self):
        ...
        self.assertIn("backend-risk-v1-authorized-no-backend-ci-publication-http-retry-20260910", workflow)
...
class ActualGateShellTests(unittest.TestCase):
    def test_registered_keys_and_fail_closed_states(self):
        import yaml
        workflow = yaml.safe_load((ROOT / '.github/workflows/ci.yml').read_text())
        job = next(j for j in workflow['jobs'].values() if j.get('name') == 'Backend risk gate')
        shell = next(step['run'] for step in job['steps'] if step.get('name') == 'Require the classified backend evidence')
        baseline = dict(os.environ, MODE='authorized-no-backend-ci', POLICY_RESULT='success', REUSE_BACKEND='false', FULL_RESULT='skipped', FOCUSED_RESULT='skipped', REUSE_SOURCE_RUN='')
        for key in (POLICY.CARDIOL_GATE_SCOPE['suite_key'], POLICY.CORPUS_REPAIR_SCOPE['suite_key']):
            env = dict(baseline, SUITE_KEY=key)
            with self.subTest(key=key):
                result = subprocess.run(['bash','-c',shell],env=env,capture_output=True,text=True)
                self.assertEqual(result.returncode,0,result.stdout+result.stderr)
        for values in ({'SUITE_KEY':'unregistered'}, {'POLICY_RESULT':'failure'}, {'REUSE_BACKEND':'true'}, {'FULL_RESULT':'success'}, {'FOCUSED_RESULT':'success'}):
            env = dict(baseline,SUITE_KEY=POLICY.CORPUS_REPAIR_SCOPE['suite_key']);env.update(values)
