...
class AuditTudoComTudoGateTests(unittest.TestCase):
    ...
    def test_editorial_gate_blocks_pending_published_and_contradictory_note(self):
        ...
        ], approved={"publicado-sem-revisao"})
    ...
    def test_versioned_approval_never_substitutes_scientific_review(self):
        record = {
        ...
        self.assertEqual([
            issue["reason"] for issue in AUDITOR._editorial_issues(
            "evidencia",
            [record],
            approved={"aprovado-em-manifesto"},
        )], ["published_without_review"])

    def test_publication_requires_versioned_approval_in_addition_to_review(self):
        record = {
            "slug": "revisado-sem-aprovacao",
            "review_status": "revisado",
            "published": True,
        }
        ...
        self.assertEqual([
            issue["reason"] for issue in AUDITOR._editorial_issues(
                "evidencia", [record], approved=set()
            )
        ], ["published_without_approval"])

    def test_approval_targets_must_exist_once_in_the_declared_front(self):
        approvals = {
            "evidencia": {
                "evidencia-ok",
                "ausente",
        ...
        records = {
            ...
            "estudo": [{"slug": "slug-no-tipo-errado"}],
        }

        issues = AUDITOR._approval_target_issues(approvals, records)
        self.assertEqual(
            {issue["identifier"]: issue["reason"] for issue in issues},
            {
                "ausente": "approval_target_not_found",
                "slug-no-tipo-errado": "approval_target_wrong_type",
                "evidencia-ambigua": "approval_target_ambiguous",
            },
        )

    def test_approval_manifest_allows_retention_but_requires_review_for_publication(self):
        records = {
            "evidencia": [
                {
                    "slug": "publica-revisada",
                    "review_status": "revisado",
                    "published": True,
                },
                {
                    "slug": "aprovada-retida",
                    "review_status": "pendente_revisao",
                    "published": False,
                },
                {
                    "slug": "publica-pendente",
                    "review_status": "pendente_revisao",
                    "published": True,
                },
            ],
        ...
        with tempfile.TemporaryDirectory() as directory:
            manifest = Path(directory) / "approval.json"
            ...
            issues, metadata = AUDITOR._approval_manifest_issues(manifest, records)

        self.assertEqual(
            [(issue["identifier"], issue["reason"]) for issue in issues],
            [("publica-pendente", "approved_publication_not_reviewed")],
        )
        self.assertEqual(metadata["item_count"], 3)
        self.assertEqual(metadata["public_items"], 2)
        self.assertEqual(metadata["retained_items"], 1)

    def test_lacuna_declarada_is_valid_but_never_publication_ready(self):
        record = {
            "slug": "lacuna-documentada",
            "review_status": "lacuna_declarada",
            "published": False,
        }
        self.assertEqual(AUDITOR._editorial_issues("doenca", [record]), [])
        self.assertEqual(
            AUDITOR._strict_release_issues("doenca", [record])[0]["blockers"],
            ["not_reviewed", "not_published"],
        )
