...
with patch.object(sys, "path", [str(ROOT / "scripts"), *sys.path]):
    SPEC = importlib.util.spec_from_file_location("ci_link_contract_owner_policy", ROOT / "scripts/ci_backend_followup.py")
    POLICY = importlib.util.module_from_spec(SPEC)
    SPEC.loader.exec_module(POLICY)


def cardiol_decision(root, **context):
    return POLICY.scoped_owner_decision(root, scope=POLICY.LINK_CONTRACT_SCOPE, **context)


class LinkContractOwnerDecisionTests(unittest.TestCase):
    def setUp(self):
        self.directory = tempfile.TemporaryDirectory()
        self.addCleanup(self.directory.cleanup)
        self.root = Path(self.directory.name)
        self.manifest = json.loads((ROOT / POLICY.LINK_CONTRACT_SCOPE['manifest']).read_text())
        self.manifest["pull_request"] = 999
        self.write_manifest()
        for path in ["docs/cardiol-link-contracts-20260910.md", "docs/ci/cardiol-link-contracts-owner-decision.md"]:
        ...
        self.pr = {"number": 999, "base": {"ref": "main", "repo": {"full_name": POLICY.LINK_CONTRACT_SCOPE['repository']}},
            "head": {"ref": POLICY.LINK_CONTRACT_SCOPE['head_branch'], "sha": self.head,
                     "repo": {"full_name": POLICY.LINK_CONTRACT_SCOPE['repository']}},
            "merged": False, "merge_commit_sha": None}
        self.associated = []
        self.calls = []

    def write_manifest(self):
        path = self.root / POLICY.LINK_CONTRACT_SCOPE['manifest']
    ...
    def decide(self, *, event="pull_request", number="999", head_ref=None):
        with patch.object(POLICY, "github", self.github), patch.object(POLICY, "git", self.git), \
            ...
            result = cardiol_decision(self.root, event=event, number=number,
                head_ref=POLICY.LINK_CONTRACT_SCOPE['head_branch'] if head_ref is None else head_ref,
                repository=POLICY.LINK_CONTRACT_SCOPE['repository'])
        ...
        try:
            with patch.object(sys, "argv", ["ci_backend_followup.py", "--repo-root", str(self.root),
                 ...
                 patch.dict(os.environ, {"EVENT_NAME": "pull_request", "PR_NUMBER": "999",
                    "PR_HEAD_REF": POLICY.LINK_CONTRACT_SCOPE['head_branch'], "REPOSITORY": POLICY.LINK_CONTRACT_SCOPE['repository']}), \
        ...
        with patch.object(POLICY, "git", self.git), \
            ...
            with self.assertRaises(subprocess.CalledProcessError):
                cardiol_decision(self.root, event="pull_request", number="999",
                    head_ref=POLICY.LINK_CONTRACT_SCOPE['head_branch'], repository=POLICY.LINK_CONTRACT_SCOPE['repository'])
    ...
    def test_wrong_head_repo_or_stale_head_never_authorizes(self):
        ...
        with self.assertRaises(ValueError): self.decide()
        self.pr["head"]["repo"]["full_name"] = POLICY.LINK_CONTRACT_SCOPE['repository']
        self.candidate = "b" * 40
        with self.assertRaises(ValueError): self.decide()
...
class ActualGateShellTests(unittest.TestCase):
    def test_registered_keys_and_fail_closed_states(self):
        ...
        for key in (POLICY.CORPUS_REPAIR_SCOPE['suite_key'], POLICY.LINK_CONTRACT_SCOPE['suite_key']):
        ...
        for values in ({'SUITE_KEY':'unregistered'}, {'POLICY_RESULT':'failure'}, {'REUSE_BACKEND':'true'}, {'FULL_RESULT':'success'}, {'FOCUSED_RESULT':'success'}):
            env = dict(baseline,SUITE_KEY=POLICY.LINK_CONTRACT_SCOPE['suite_key']);env.update(values)
