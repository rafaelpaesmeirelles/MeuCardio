...
with patch.object(sys, "path", [str(ROOT / "scripts"), *sys.path]):
    SPEC = importlib.util.spec_from_file_location("ci_favorites_owner_policy", ROOT / "scripts/ci_backend_followup.py")
    POLICY = importlib.util.module_from_spec(SPEC)
    SPEC.loader.exec_module(POLICY)


class FavoritesOwnerDecisionTests(unittest.TestCase):
    def setUp(self):
        self.directory = tempfile.TemporaryDirectory()
        self.addCleanup(self.directory.cleanup)
        self.root = Path(self.directory.name)
        self.manifest = json.loads((ROOT / POLICY.FAVORITES_MANIFEST).read_text())
        self.manifest["pull_request"] = 999
        self.write_manifest()
        for path in ["docs/universal-favorites-audit-20260910.md", "docs/ci/universal-favorites-owner-decision.md"]:
        ...
        self.pr = {"number": 999, "base": {"ref": "main", "repo": {"full_name": POLICY.FAVORITES_REPOSITORY}},
            "head": {"ref": POLICY.FAVORITES_BRANCH, "sha": self.head,
                     "repo": {"full_name": POLICY.FAVORITES_REPOSITORY}},
            "merged": False, "merge_commit_sha": None}
        self.associated = []
        self.calls = []

    def write_manifest(self):
        path = self.root / POLICY.FAVORITES_MANIFEST
    ...
    def decide(self, *, event="pull_request", number="999", head_ref=None):
        with patch.object(POLICY, "github", self.github), patch.object(POLICY, "git", self.git), \
            ...
            result = POLICY.universal_favorites_owner_decision(self.root, event=event, number=number,
                head_ref=POLICY.FAVORITES_BRANCH if head_ref is None else head_ref,
                repository=POLICY.FAVORITES_REPOSITORY)
        ...
        try:
            with patch.object(sys, "argv", ["ci_backend_followup.py", "--repo-root", str(self.root),
                 ...
                 patch.dict(os.environ, {"EVENT_NAME": "pull_request", "PR_NUMBER": "999",
                    "PR_HEAD_REF": POLICY.FAVORITES_BRANCH, "REPOSITORY": POLICY.FAVORITES_REPOSITORY}), \
    ...
    def test_api_failure_propagates_without_selecting_suite(self):
        with patch.object(POLICY, "git", self.git), \
            ...
            with self.assertRaises(subprocess.CalledProcessError):
                POLICY.universal_favorites_owner_decision(self.root, event="pull_request", number="999",
                    head_ref=POLICY.FAVORITES_BRANCH, repository=POLICY.FAVORITES_REPOSITORY)
    ...
    def test_identity_origin_and_missing_authorization_fail_closed(self):
        for field, value in [("base_sha", "0" * 40), ("repository", "other/repo"),
                             ("head_branch", "other-branch"), ("pull_request", 918), ("pull_request", 919), ("pull_request", 920), ("pull_request", 921)]:
    ...
    def test_wrong_head_repo_or_stale_head_never_authorizes(self):
        ...
        with self.assertRaises(ValueError): self.decide()
        self.pr["head"]["repo"]["full_name"] = POLICY.FAVORITES_REPOSITORY
        self.candidate = "b" * 40
        with self.assertRaises(ValueError): self.decide()
    ...
    def test_workflow_skips_both_test_jobs_and_keeps_explicit_gate_label(self):
        ...
        self.assertIn("backend-risk-v1-authorized-no-backend-ci-universal-favorites-20260910", workflow)
