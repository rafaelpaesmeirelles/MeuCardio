...
with patch.object(sys, "path", [str(ROOT / "scripts"), *sys.path]):
    SPEC = importlib.util.spec_from_file_location("ci_http_retry_owner_policy", ROOT / "scripts/ci_backend_followup.py")
    POLICY = importlib.util.module_from_spec(SPEC)
    SPEC.loader.exec_module(POLICY)


class HttpRetryOwnerDecisionTests(unittest.TestCase):
    def setUp(self):
        self.directory = tempfile.TemporaryDirectory()
        self.addCleanup(self.directory.cleanup)
        self.root = Path(self.directory.name)
        self.manifest = json.loads((ROOT / POLICY.HTTP_RETRY_MANIFEST).read_text())
        self.manifest["pull_request"] = 999
        self.write_manifest()
        for path in ["docs/publication-http-retry-audit-20260910.md", "docs/ci/publication-http-retry-owner-decision.md"]:
        ...
        self.pr = {"number": 999, "base": {"ref": "main", "repo": {"full_name": POLICY.HTTP_RETRY_REPOSITORY}},
            "head": {"ref": POLICY.HTTP_RETRY_BRANCH, "sha": self.head,
                     "repo": {"full_name": POLICY.HTTP_RETRY_REPOSITORY}},
            "merged": False, "merge_commit_sha": None}
        self.associated = []
        self.calls = []

    def write_manifest(self):
        path = self.root / POLICY.HTTP_RETRY_MANIFEST
    ...
    def decide(self, *, event="pull_request", number="999", head_ref=None):
        with patch.object(POLICY, "github", self.github), patch.object(POLICY, "git", self.git), \
            ...
            result = POLICY.publication_http_retry_owner_decision(self.root, event=event, number=number,
                head_ref=POLICY.HTTP_RETRY_BRANCH if head_ref is None else head_ref,
                repository=POLICY.HTTP_RETRY_REPOSITORY)
        ...
        try:
            with patch.object(sys, "argv", ["ci_backend_followup.py", "--repo-root", str(self.root),
                 ...
                 patch.dict(os.environ, {"EVENT_NAME": "pull_request", "PR_NUMBER": "999",
                    "PR_HEAD_REF": POLICY.HTTP_RETRY_BRANCH, "REPOSITORY": POLICY.HTTP_RETRY_REPOSITORY}), \
    ...
    def test_api_failure_propagates_without_selecting_suite(self):
        with patch.object(POLICY, "git", self.git), \
            ...
            with self.assertRaises(subprocess.CalledProcessError):
                POLICY.publication_http_retry_owner_decision(self.root, event="pull_request", number="999",
                    head_ref=POLICY.HTTP_RETRY_BRANCH, repository=POLICY.HTTP_RETRY_REPOSITORY)
    ...
    def test_wrong_head_repo_or_stale_head_never_authorizes(self):
        ...
        with self.assertRaises(ValueError): self.decide()
        self.pr["head"]["repo"]["full_name"] = POLICY.HTTP_RETRY_REPOSITORY
        self.candidate = "b" * 40
        with self.assertRaises(ValueError): self.decide()
