...
VALID_REVIEW_STATUSES = EXPLICIT_REVIEW_STATUSES
...
def _editorial_issues(
    ...
    *,
    ...
    for index, record in enumerate(records):
        ...
        if published is True and status != "revisado" and identifier not in approved:
            issues.append({
                "kind": kind,
                "identifier": identifier,
                "reason": "published_without_review_or_approval",
                "review_status": status,
                "published": published,
            })
...
def _gate_summary(
    ...
    blocker_count = len(reference_issues) + len(editorial_issues) + len(release_issues)
    summary = {
...
def audit(
    *,
    ...
    if strict_release and strict_release_manifest is not None:
        raise ValueError(
            "Use apenas um modo estrito: corpus completo ou manifesto de release"
        )
    ...
    if strict_release_manifest is not None:
        release_readiness_issues, release_manifest = (
            _strict_release_manifest_issues(
                ...
                {**manifests, "documento_markdown": document_records},
            )
        )
