#!/usr/bin/env python3
"""Revisor independente do lote 3 Grok. Números ⊆ abstract PubMed."""
...
ge = [r for r in estudos if r.get("fonte_producao") == "grok" and (r.get("review_note") or "").startswith("Lote 3")]
gv = [r for r in evidencias if r.get("fonte_producao") == "grok" and (r.get("review_note") or "").startswith("Lote 3")]
...
for p, n in Counter(grok_pmids).items():
    if n > 1:
        errors.append(("dup_lote3_pmid", p))
for p in grok_pmids:
    if p in base_pmids:
...
print("INDEPENDENT REVIEW lote 3")
print("estudos_lote3", len(ge), "evidencias_lote3", len(gv), "keep", len(KEEP))
