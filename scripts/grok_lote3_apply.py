#!/usr/bin/env python3
"""Aplica lote 3 Grok (estudos + evidências) no clone local. Não publica. Não mexe em main."""
...
KEEP = json.loads(Path("/tmp/pubmed_lote3/keep.json").read_text())
SYNTH = json.loads(Path("/tmp/lote3_synth.json").read_text())
...
NOTE_E = (
    "Lote 3 Grok 2026-08-30. Síntese portuguesa original a partir do abstract PubMed (efetch). "
...
NOTE_V = (
    "Lote 3 Grok 2026-08-30. Não é classe de diretriz ESC/AHA/SBC. Derivado do ensaio-índice "
    "(abstract). Manter pendente_revisao até casar com tabela oficial e revisão independente."
)
NOTE_V2 = (
    "Lote 3 Grok 2026-08-30. Segunda evidência do mesmo ensaio-índice (segurança, secundário ou "
