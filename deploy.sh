...
[[ "$RUNNING_SHA" =~ ^[0-9a-f]{40}$ ]] || {
  echo "Não foi possível confirmar o SHA do backend já em execução; recusando hotfix isolado." >&2
  exit 65
}
git cat-file -e "${RUNNING_SHA}^{commit}" 2>/dev/null || {
  echo "Commit atualmente em produção não existe no checkout local." >&2
  exit 65
}
...
# Compila somente o frontend do SHA exato e copia o dist para o volume sitefiles
# já lido pelo Caddy. Nenhum serviço clínico é recriado ou reiniciado.
DEPLOY_COMMIT="$CURRENT_SHA" "${COMPOSE[@]}" build frontend-build
DEPLOY_COMMIT="$CURRENT_SHA" "${COMPOSE[@]}" run --rm --no-deps frontend-build

# Certifica que o bundle publicado contém o SHA exato do frontend.
