...
# Nesta publicação consolidada, `CORVIA_SKIP_RAG_REINDEX` fica habilitado por
# padrão porque a cota do provedor está esgotada. A chamada permanece disponível
# para uma execução futura explícita, sem duplicar embeddings neste deploy.
...
if [[ "${AI_ENABLED:-false}" == "true" && "${CORVIA_SKIP_RAG_REINDEX:-true}" != "true" ]]; then
