"""Contrato do verbete NOVO "insuficiencia-mitral" (área geral) criado em
29/08/2026 via doencas/fragmentos/insuficiencia-mitral.json — valvopatia
mais prevalente no mundo ocidental, sem ficha própria até então apesar de
corpus rico (COAPT, MITRA-FR, EVEREST II, disjunção anular mitral).

Nota: todos os 8 PMIDs verificados via NCBI e-utils antes da montagem (1
referência adicional sem PMID, apenas DOI, mantida como tal).
...
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    "fluxograma-regurgitacao-mitral-esc-eacts-2025",
    "reparo-percutaneo-borda-a-borda-versus-cirurgia-na-regurgitacao-mitral-primaria-everest-ii-5-anos",
    "regurgitacao-mitral-secundaria-transcateter-coapt-versus-mitra-fr-por-que-resultados-divergiram",
    "disjuncao-anular-mitral-e-prolapso-valvar-mitral-arritmico",
}  # todos também em valvopatias (hub geral com 41 related_document_slugs)
...
def test_catalogacao():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Insuficiência mitral"
    assert "Regurgitação mitral" in (item.get("aliases") or [])
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "regurgitação", "válvula" if "válvula" in serialized else "prolapso"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(t in texto for t in TERMOS_TEMA), f"{slug}: não menciona mitral"
...
def test_patient_material_slug_resolve():
    ...
    assert material == "regurgitacao-mitral-o-que-e-e-quando-precisa-ser-corrigida"
    materiais = {
        x["slug"]
