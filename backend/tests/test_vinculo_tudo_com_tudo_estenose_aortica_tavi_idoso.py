...
ficha já existente estenose-aortica-tavi-idoso (área cardiogeriatria) em
doencas/metadados.json.

Vigésimo oitavo lote de conteúdo do dia, segundo desta natureza (após
doenca-de-kawasaki, PR #642): não é aprofundamento de conteúdo clínico. A
ficha já estava completeness=completo, com epidemiology/
diagnostic_approach/treatment_summary/assistant_rules integrais e
review_status=revisado — mas related_document_slugs e
patient_material_slug estavam None, violação pontual da regra Tudo com
Tudo (3-7 vínculos obrigatórios).
...
SLUG = "estenose-aortica-tavi-idoso"

PASTAS_NAO_DOCUMENTO = ("Farmacologia", "Calculadoras", "Exames")

TERMOS_TEMA = ("estenose aórtica", "tavi", "tavr")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em fragilidade-pre-procedimento-cardiovascular,
    # avaliacao-multidimensional-cardiogeriatrica e valvopatias (o
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Estenose aórtica e TAVI no idoso"
    assert "TAVI geriátrica" in (item.get("aliases") or [])
...
def test_related_document_slugs_mencionam_estenose_aortica_ou_tavi():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona estenose aórtica/TAVI no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "o-que-esperar-da-troca-de-valvula-por-cateter-tavi"
    materiais = {
        x["slug"]
