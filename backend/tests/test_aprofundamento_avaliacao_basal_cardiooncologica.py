...
já existente avaliacao-basal-cardiooncologica (área cardiooncologia) em
doencas/metadados.json.

Trigésimo terceiro lote de conteúdo do dia. Ficha estava rotulada
...
preenche apenas as lacunas identificadas, conectando a ficha a 5 itens já
...
Nota de compliance: a sigla "HFA-ICOS" aparece uma única vez em
epidemiology, citando o nome da ferramenta de estratificação de risco
(nunca em assistant_rules nem special_populations, escopo real do gate
de compliance).
"""
...
SLUG = "avaliacao-basal-cardiooncologica"
...
TERMOS_TEMA = ("avaliação basal", "cardio-oncologia", "cardiotoxicidade", "risco basal")
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Avaliação cardiovascular basal antes do tratamento oncológico"
    assert "baseline cardio-oncology" in (item.get("aliases") or [])
    assert item.get("category") == "avaliacao_basal"
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "avaliação", "vigilância"):
...
def test_assistente_deterministico_seguro():
    ...
    assert "hfa-icos" not in serialized, "assistant_rules não pode conter 'hfa-icos' (gate real de compliance)"
...
def test_epidemiology_cita_hfa_icos_no_maximo_uma_vez_fora_de_special_populations_e_rules():
    """A sigla HFA-ICOS pode nomear a ferramenta em epidemiology, mas não
    deve se espalhar para diagnostic_approach/treatment_summary/monitoring
    nem para os campos de compliance real (já cobertos acima)."""
    item = _load_doencas()[SLUG]
    outros_campos = json.dumps(
        [item.get("diagnostic_approach"), item.get("treatment_summary"), item.get("monitoring")],
        ensure_ascii=False,
    ).casefold()
    assert "hfa-icos" not in outros_campos
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona avaliação basal/cardio-oncologia no texto"
        )
