...
já existente indicacoes-ecocardiograma-fetal (área cardiopediatria) em
...
seguimento-cardiovascular-pos-parto PR #628). Ficha já tinha
patient_material_slug e 1 related_document_slug preenchidos, mas zero
campos clínicos.

Ficha framework/protocolo de triagem (indicações para ecocardiograma
fetal direcionado), não doença única — presentation/differentials
foram reinterpretados como categorias de indicação/distinções
conceituais, mesmo padrão já usado em avaliacao-multidimensional-
cardiogeriatrica/PR #613, cuidados-paliativos-cardiovasculares/PR #615
e plano-parto-cardiopatia-materna/PR #626.
...
fora de Farmacologia/Calculadoras/Exames (busca exaustiva encontrou
apenas 3 documentos narrativos centrais ao tema; várias fichas de
doença fetal específica que mencionam o exame foram deliberadamente
excluídas por não serem "documentos" no sentido da regra Tudo com
Tudo — são outras entradas do catálogo de doenças, não markdown
narrativo em content/).
...
SLUG = "indicacoes-ecocardiograma-fetal"
...
TERMOS_TEMA = (
    "ecocardiograma fetal", "ecocardiografia fetal", "eco fetal",
    "risco de recorrência", "indicação",
)
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Indicações de ecocardiograma fetal"
    assert "quando pedir eco fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "rastreamento_e_indicacoes"
    assert item.get("prevalence_rank") == 31
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "ecocardiograma"):
...
def test_related_document_slugs_mencionam_eco_fetal_ou_indicacao():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona ecocardiograma fetal "
            "nem risco de recorrência/indicação no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "indicacoes-ecocardiograma-fetal"
    materiais = {
        x["slug"]
