"""Contrato do aprofundamento Tudo com Tudo de indicações de ecocardiograma fetal — 28/08/2026."""
from __future__ import annotations
import json,re
from pathlib import Path
from app.services.clinical_rule_engine import validate_question_definitions,validate_rule_definitions
REPOSITORY_ROOT=Path(__file__).resolve().parents[2]; DOENCAS_PATH=REPOSITORY_ROOT/"doencas/metadados.json"; SLUG="indicacoes-ecocardiograma-fetal"
...
PASTAS_NAO_DOCUMENTO=("Farmacologia","Calculadoras","Exames"); DOSE_PATTERNS=(r"\d+[\.,]?\d*\s*mg(?!/d[lL])\b",r"\d+[\.,]?\d*\s*mg/kg",r"\d+[\.,]?\d*\s*mcg",r"\d+[\.,]?\d*\s*j/kg"); ALLOWED_ADD_KEYS={"risk","red_flags","supporting","opposing","missing_information","suggested_tests","differentials","ambulatory_flow","emergency_flow","messages"}; TERMOS_TEMA=("ecocardiograma fetal","ecocardiografia fetal","eco fetal","risco de recorrência","indicação")
...
def test_catalogacao_original_preservada():
 i=_load_doencas()[SLUG]; assert i.get("name")=="Indicações de ecocardiograma fetal"; assert "quando pedir eco fetal" in (i.get("aliases") or []); assert i.get("category")=="cardiologia_fetal"; assert i.get("subtype")=="rastreamento_e_indicacoes"; assert i.get("prevalence_rank")==31
...
def test_texto_com_acentuacao_correta_do_portugues():
 ...
 for p in ("não ","cardíaca","gestação","ecocardiograma"): assert p in s
...
def test_patient_material_slug_resolve():
 i=_load_doencas()[SLUG]; m=i.get("patient_material_slug"); assert m=="indicacoes-ecocardiograma-fetal"; assert m in {x["slug"] for x in json.loads((REPOSITORY_ROOT/"material-paciente/metadados.json").read_text(encoding="utf-8"))}