"""Contrato do aprofundamento Tudo com Tudo de taquicardia supraventricular fetal — 28/08/2026."""
from __future__ import annotations
import json,re
from pathlib import Path
from app.services.clinical_rule_engine import validate_question_definitions,validate_rule_definitions
REPOSITORY_ROOT=Path(__file__).resolve().parents[2]; DOENCAS_PATH=REPOSITORY_ROOT/"doencas/metadados.json"; SLUG="taquicardia-supraventricular-fetal"
...
def test_patient_material_slug_resolve():
 i=_load_doencas()[SLUG]; m=i.get("patient_material_slug"); assert m=="taquicardia-supraventricular-fetal"; assert m in {x["slug"] for x in json.loads((REPOSITORY_ROOT/"material-paciente/metadados.json").read_text(encoding="utf-8"))}