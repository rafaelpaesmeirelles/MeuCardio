...
já existente taquicardia-supraventricular-fetal (área cardiopediatria) em
doencas/metadados.json.

Vigésimo sexto lote de conteúdo do dia. Ficha já tinha
patient_material_slug e 1 related_document_slug preenchidos, além de
presentation/differentials/tests/red_flags básicos em bullet
(completeness=intermediario), mas zero epidemiology, diagnostic_approach,
treatment_summary ou assistant machinery (1 source_ref).

O lote não cria nenhum documento, checklist, trilha ou material novo —
conecta a ficha a 3 itens já publicados e revisados em content/, todos
fora de Farmacologia/Calculadoras/Exames. Este é o piso mínimo permitido
pela regra Tudo com Tudo (3): antes de dispatch, verifiquei por leitura
manual (não apenas grep) que os 2 novos candidatos mencionam o tema de
forma central e positiva, e um agente de pesquisa dedicado confirmou
independentemente os mesmos 2 candidatos, sem achar nenhum adicional no
corpus — não fabriquei uma quarta conexão para "parecer mais robusto".

Nota de transparência: dois alvos anteriores desta rodada foram
descartados antes de qualquer dispatch de pesquisa — extrassistoles-
fetais (corpus insuficiente: candidatos com menção zero, tangencial, ou
negação explícita de conexão) e cardiotoxicidade-inibidor-proteassoma
(colisão real confirmada com a PR #551 aberta, via gh pr diff).
...
SLUG = "taquicardia-supraventricular-fetal"
...
TERMOS_TEMA = ("taquicardia supraventricular", "tsv fetal", "taquiarritmia fetal")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em bloqueio-atrioventricular-fetal, flutter-atrial-fetal e
    # hidropisia-fetal-cardiovascular (mesmo documento-fonte de arritmias
    # fetais, cobrindo TSV e BAVT)
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Taquicardia supraventricular fetal"
    assert "TSV fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "arritmia_fetal"
    assert item.get("prevalence_rank") == 9
...
def test_related_document_slugs_mencionam_tsv_fetal():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona TSV/taquiarritmia fetal no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "taquicardia-supraventricular-fetal"
    materiais = {
        x["slug"]
