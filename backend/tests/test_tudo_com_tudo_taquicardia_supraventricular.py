...
Taquicardia supraventricular (área geral) em doencas/metadados.json.

Décimo terceiro ciclo independente do dia (após endocardite infecciosa
...
venoso PR #574, doença arterial periférica de membros PR #578 e doença da
aorta PR #580). Não havia hub geral de taquicardia supraventricular (TSV)
do adulto — apenas o hub-irmão taquicardia-supraventricular-fetal (área
cardiopediatria), preservado sem duplicação. O lote não cria nenhum
documento, checklist, trilha ou material novo — conecta o novo verbete a
12 itens já publicados e revisados em content/Arritmias/ e adjacentes,
todos fora de Farmacologia/Calculadoras/Exames.

Nota de compliance: na montagem, um item de special_populations produzido
pelo agente de pesquisa incluía dose de adenosina em mg/kg e energia de
cardioversão em J/kg para a população pediátrica — removido e reescrito em
termos qualitativos antes do commit, respeitando a proibição categórica de
doses de fármaco em qualquer campo do corpus. Este teste verifica
programaticamente a ausência de padrões de dose em todo o registro.
"""
...
NOVO_SLUG = "taquicardia-supraventricular"
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 8, "hub de TSV deveria conectar um volume relevante de documentos"
...
def test_related_document_slugs_sao_todos_sobre_taquicardia_supraventricular():
    """Vínculo direto: todo documento conectado deve mencionar taquicardia
    supraventricular/AVNRT/AVRT/WPW explicitamente no próprio texto — evita
    link por proximidade temática."""
    item = _load_doencas()[NOVO_SLUG]
    documentos = _all_document_paths()
    termos = (
        "taquicardia supraventricular", "avnrt", "avrt", "wolff-parkinson", "sanrt",
        "taquicardia atrial", "taquicardia juncional", "taquicardia sinusal",
        "taquicardiomiopatia", "palpita",
    )
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in termos), (
            f"{slug}: documento vinculado não menciona TSV/AVNRT/AVRT/WPW no texto"
        )
