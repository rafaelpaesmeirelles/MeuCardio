...
já existente tetralogia-fallot-fetal (área cardiopediatria) em
...
PR #636). Ficha já tinha patient_material_slug e 1
related_document_slug preenchidos (o único vínculo original trata
majoritariamente de manejo pós-natal, não do período fetal), mas zero
campos clínicos.
...
nenhuma correção foi necessária (diferente das duas rodadas
anteriores, coarctacao-aorta-fetal/PR #636 e hidropisia-fetal-
cardiovascular/PR #632, que tiveram erros de citação corrigidos).

Nota sobre sobreposição: overlap extenso mas legítimo com fichas já
existentes sobre o mesmo tema (tetralogia de Fallot no adulto, atresia
pulmonar, indicações de eco fetal, planejamento de parto em
cardiopatia fetal), documentado abaixo.
...
TERMOS_TEMA = ("tetralogia de fallot", "fallot")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em tetralogia-de-fallot (adulto)
    "tetralogia-de-fallot-rastreio-pre-natal-crise-de-hipoxia-e-estrategia-cirurgica",
    "crise-hipercianotica-na-tetralogia-de-fallot",
    # também em atresia-pulmonar
    "atresia-pulmonar-anatomia-dependencia-coronariana-e-estrategia-cirurgica",
    # também em indicacoes-ecocardiograma-fetal (PR #630)
    "risco-de-recorrencia-de-cardiopatia-congenita-na-prole-e-ecocardiograma-fetal-direcionado",
    # também em planejamento-parto-cardiopatia-fetal
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Tetralogia de Fallot no período fetal"
    assert "Fallot fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "conotruncal"
    assert item.get("prevalence_rank") == 38
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "pulmonar"):
...
def test_related_document_slugs_mencionam_fallot():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona tetralogia de Fallot no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "tetralogia-fallot-fetal"
    materiais = {
        x["slug"]
