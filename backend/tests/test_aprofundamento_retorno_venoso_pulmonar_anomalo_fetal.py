...
já existente retorno-venoso-pulmonar-anomalo-fetal (área cardiopediatria)
em doencas/metadados.json.

Vigésimo quarto lote de conteúdo do dia, oitavo do cluster de cardiologia
fetal (após indicacoes-ecocardiograma-fetal PR #630, bloqueio-
atrioventricular-fetal PR #631, hidropisia-fetal-cardiovascular PR #632,
flutter-atrial-fetal PR #633, coarctacao-aorta-fetal PR #636, tetralogia-
fallot-fetal PR #637, transposicao-grandes-arterias-fetal PR #638). Ficha
já tinha patient_material_slug e 1 related_document_slug preenchidos, mas
zero campos clínicos.

Alvo anterior deste ciclo (tumores-cardiacos-fetais) foi abandonado antes
de qualquer commit por não atingir o piso de 3 related_document_slugs
exigido pela regra Tudo com Tudo (apenas 2 candidatos legítimos
identificados no corpus) — decisão de não fabricar uma terceira conexão.
...
verificados individualmente via NCBI e-utils antes da montagem, tanto pelo
agente de pesquisa quanto de forma independente por mim — nenhuma correção
foi necessária.

Nota clínica de segurança: diferente de cardiopatias ducto-dependentes, a
prostaglandina E1 não tem papel terapêutico útil na forma obstrutiva do
RVPAT e pode agravar a congestão pulmonar — reforçado em treatment_summary,
emergency_flow e em duas assistant_rules dedicadas.

Nota sobre sobreposição: overlap parcial mas legítimo com a ficha adulta
retorno-venoso-pulmonar-anomalo (alvo do PR #576 aberto, que não toca esta
ficha -fetal, confirmado via gh pr diff) — documentado abaixo.
...
SLUG = "retorno-venoso-pulmonar-anomalo-fetal"
...
TERMOS_TEMA = ("retorno venoso pulmonar anômalo", "retorno venoso pulmonar anomalo", "rvpat", "rvpa", "tapvr", "tapvc")
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Retorno venoso pulmonar anômalo no período fetal"
    assert "RVPAT fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "conexoes_venosas"
    assert item.get("prevalence_rank") == 40
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "veia"):
...
def test_prostaglandina_sem_papel_util_na_forma_obstrutiva():
    """Nota clínica de segurança específica desta ficha: ao contrário de
    lesões ducto-dependentes, a prostaglandina E1 não beneficia a forma
    obstrutiva do RVPAT e pode agravar a congestão pulmonar."""
    item = _load_doencas()[SLUG]
    serialized = json.dumps(
        [item.get("treatment_summary"), item.get("emergency_flow"), item.get("assistant_rules")],
        ensure_ascii=False,
    ).casefold()
    assert "prostaglandina" in serialized
    assert "não tem papel terapêutico útil" in serialized or "não deve ser considerada medida" in serialized
...
def test_related_document_slugs_mencionam_rvpat():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona retorno venoso "
            "pulmonar anômalo/RVPAT no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "retorno-venoso-pulmonar-anomalo-fetal"
    materiais = {
        x["slug"]
