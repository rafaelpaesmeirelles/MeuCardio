...
Diabetes mellitus tipo 2 e risco cardiovascular (área geral) em
doencas/metadados.json.

Nono ciclo independente do dia (após endocardite infecciosa PR #553,
pericardite PR #554, hipertensão pulmonar PR #555, síncope PR #560,
valvopatias PR #563, cardiomiopatias PR #565, miocardite PR #568 e
dislipidemia PR #570). Não havia nenhum verbete-hub geral de diabetes e
risco cardiovascular, apesar de content/Diabetes_e_cardiologia/ reunir 63
documentos narrativos dedicados ao tema. Escopo deliberadamente restrito ao
risco/desfecho cardiovascular do diabetes tipo 2 — não é verbete de
endocrinologia geral sobre manejo primário de glicemia. O lote não cria
nenhum documento, checklist, trilha ou material novo — conecta o novo
verbete a 34 itens já publicados e revisados em content/, todos fora de
...
NOVO_SLUG = "diabetes-mellitus-tipo-2"
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 20, "hub de diabetes deveria conectar um volume relevante de documentos"
...
def test_related_document_slugs_sao_todos_sobre_diabetes():
    """Vínculo direto: todo documento conectado deve mencionar diabetes
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert "diabet" in texto, f"{slug}: documento vinculado não menciona diabetes no texto"
