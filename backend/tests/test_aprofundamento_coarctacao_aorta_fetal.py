...
já existente coarctacao-aorta-fetal (área cardiopediatria) em
...
PR #632, flutter-atrial-fetal PR #633). Ficha já tinha
...
verificados individualmente via NCBI e-utils antes da montagem. Duas
correções foram necessárias: (1) a referência ASE 2023 tinha autor
incorreto no texto entregue pelo agente ("Lopez L, Saurers DL, Barker
PCA") — corrigido para o autor real (Moon-Grady AJ), mesmo já usado
corretamente em indicacoes-ecocardiograma-fetal/PR #630; (2) duas
referências (Tuo G et al. 2022, Wang HH et al. 2022) foram entregues
sem nome de autor — completadas após confirmação via NCBI.

Nota sobre sobreposição: overlap extenso mas legítimo com fichas já
existentes sobre o mesmo eixo anatômico (coarctação/valva aórtica
bicúspide/aortopatia), documentado abaixo.
...
TERMOS_TEMA = ("coarctação", "coarctacao")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em coarctacao-da-aorta (adulto)
    ...
    # também em aortopatia-na-gravidez e valva-aortica-bicuspide-pediatrica (PR #604)
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Suspeita fetal de coarctação da aorta"
    assert "coarctação fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "arco_aortico"
    assert item.get("prevalence_rank") == 37
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "coarctação"):
...
def test_related_document_slugs_mencionam_coarctacao():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona coarctação no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "coarctacao-aorta-fetal"
    materiais = {
        x["slug"]
