"""Contrato do verbete NOVO "obesidade-e-coracao" (área geral) criado em
29/08/2026 via doencas/fragmentos/obesidade-e-coracao.json — obesidade
como fator de risco cardiovascular independente, sem ficha própria até
então apesar de corpus rico (SELECT, STEP-HFpEF, SUMMIT, SOS, paradoxo da
obesidade, DOAC em obesidade extrema, estigma de peso).

Nota: todos os 10 PMIDs verificados via NCBI e-utils antes da montagem.
...
falha intencionalmente (política vigente desde 28/08/2026).
"""
...
TERMOS_TEMA = ("obesidade", "imc", "índice de massa corporal", "peso corporal")
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    "perder-peso-reduz-evento-cardiovascular-look-ahead-surmount-1-e-o-contraste-com-o-select",  # também em diabetes-mellitus-tipo-2
    "doac-em-obesidade-extrema-a-mudanca-de-orientacao-da-isth",  # também em tromboembolismo-venoso
}
...
def test_catalogacao():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Obesidade e risco cardiovascular"
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "obesidade", "circunferência"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(t in texto for t in TERMOS_TEMA), f"{slug}: não menciona obesidade/IMC/peso"
