...
já existente plano-parto-cardiopatia-materna (área gravidez) em
...
gestacao-lactacao PR #625). Ficha tinha apenas metadados de catalogação
— zero campos clínicos, zero related_document_slugs, sem
patient_material_slug.

Ficha framework/processo (planejamento multidisciplinar do parto em
gestante cardiopata), não uma doença única — presentation/differentials
foram reinterpretados como cenários de decisão/distinções conceituais,
mantendo o mesmo schema (mesmo padrão já usado em
...
Nota sobre candidato descartado: classificacao-de-risco-mwho-2-0-na-
gravidez-esc-2025 foi proposto pelo agente de pesquisa mas descartado
na montagem por tratar apenas de classificação de risco geral (classes
I a IV), sem menção específica a plano/via de parto, periparto ou local
do parto — não satisfaz o critério de centralidade ao tema desta ficha,
apesar de ser clinicamente adjacente (a classificação de risco é a base
do planejamento, mas o documento em si não discute o planejamento).
...
SLUG = "plano-parto-cardiopatia-materna"
...
TERMOS_TEMA = (
    "plano de parto", "via de parto", "cesariana", "parto vaginal",
    "periparto", "pregnancy heart team", "segunda fase do parto",
    "indução", "inducao",
)

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em protese-mecanica-na-gravidez
    "trombose-de-protese-valvar-mecanica-na-gestacao",
    "anticoagulacao-em-valva-mecanica-e-cardiomiopatia-periparto-na-gestacao",
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Plano de parto na cardiopatia materna"
    assert "delivery plan cardíaco" in (item.get("aliases") or [])
    assert item.get("category") == "planejamento"
    assert item.get("subtype") == "parto"
...
def test_related_document_slugs_mencionam_planejamento_de_parto():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona plano/via de parto, "
            "periparto nem termo relevante de planejamento de parto no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "plano-parto-cardiopatia-materna"
    materiais = {
        x["slug"]
