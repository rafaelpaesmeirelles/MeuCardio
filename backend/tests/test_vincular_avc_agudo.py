...
completa `acidente-vascular-cerebral-agudo` (área geral) em
doencas/metadados.json, que tinha apenas 2 related_document_slugs (abaixo
do piso mínimo de 3). Este lote SÓ adiciona vínculo, com foco deliberado
na interseção cardiovascular do AVC (dado que este é um sistema de
cardiologia) — não reescreve conteúdo clínico já existente.
"""
...
SLUG = "acidente-vascular-cerebral-agudo"

PASTAS_NAO_DOCUMENTO = ("Farmacologia", "Calculadoras", "Exames")
TERMOS_TEMA = ("avc", "acidente vascular cerebral", "cardioembólic", "cerebral")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em endocardite-infecciosa
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Acidente vascular cerebral agudo"
    assert item.get("area") == "geral"
    assert item.get("category") == "emergencia_neurovascular"
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona AVC/cardioembolismo/cerebral no texto"
        )
