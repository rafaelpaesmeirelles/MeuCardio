"""Contrato do lote de APROFUNDAMENTO Tudo com Tudo de 29/08/2026 — ficha já
existente planejamento-parto-cardiopatia-fetal (área cardiopediatria) em
doencas/metadados.json.

Ficha estava completeness=basico, só catalogação (1 source_ref: AHA
Scientific Statement Fetal Cardiac Disease), zero campos clínicos.
...
Nota sobre related_document_slugs: os 4 novos vínculos foram verificados
por leitura direta do documento completo. Documentos que tratam de
cardiopatia MATERNA na gestação (tema irmão mas distinto — "planejamento
do parto" decidido pela doença da mãe, não do feto) foram explicitamente
descartados.
...
review_status=='revisado' sem exceção para pendente_revisao).
"""
...
SLUG = "planejamento-parto-cardiopatia-fetal"
...
TERMOS_TEMA = ("parto", "nascimento", "perinatal", "cardiopatia fetal")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    "tetralogia-de-fallot-rastreio-pre-natal-crise-de-hipoxia-e-estrategia-cirurgica",  # também em tetralogia-de-fallot, tetralogia-fallot-fetal
    "planejamento-do-parto-na-cardiopatia-fetal-local-tempo-equipe-e-recursos-do-nascimento",  # também em tetralogia-de-fallot, tetralogia-fallot-fetal, taquicardia-supraventricular-fetal, transposicao-grandes-arterias-fetal
    "d-tga-com-mistura-atrial-restritiva-e-hipoxemia-grave-no-neonato",  # também em transposicao-das-grandes-arterias, transposicao-grandes-arterias-fetal
    "taquiarritmia-fetal-hidropsia-nao-imune-e-tratamento-transplacentario",  # também em taquicardia-supraventricular-fetal, flutter-atrial-fetal
    "retorno-venoso-pulmonar-anomalo-fetal-limitacoes-diagnosticas-e-sinais-indiretos",  # também em retorno-venoso-pulmonar-anomalo-fetal
}
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Planejamento do parto na cardiopatia fetal"
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("prevalence_rank") == 41
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "parto"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona parto/nascimento/perinatal no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "planejamento-parto-cardiopatia-fetal"
    materiais = {
        x["slug"]
