...
Dislipidemia (área geral) em doencas/metadados.json.

Oitavo ciclo independente do dia (após endocardite infecciosa PR #553,
pericardite PR #554, hipertensão pulmonar PR #555, síncope PR #560,
valvopatias PR #563, cardiomiopatias PR #565 e miocardite PR #568). Não
havia nenhum verbete-hub geral de dislipidemia do adulto — só o hub
pediátrico (dislipidemias-pediatricas). O lote não cria nenhum documento,
checklist, trilha ou material novo — conecta o novo verbete a 27 itens já
publicados e revisados em content/, todos fora de Farmacologia/Calculadoras/
Exames (9 documentos de content/Farmacologia/ propostos inicialmente pelo
agente de pesquisa foram removidos na montagem por estarem fora do escopo
permitido).
"""
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 15, "hub de dislipidemia deveria conectar um volume relevante de documentos"
...
def test_related_document_slugs_sao_todos_sobre_dislipidemia():
    """Vínculo direto: todo documento conectado deve mencionar dislipidemia/
    colesterol/lipídio explicitamente no próprio texto — evita link por
    proximidade temática."""
    item = _load_doencas()[NOVO_SLUG]
    documentos = _all_document_paths()
    termos = ("dislipidemi", "colesterol", "ldl", "lipoprote", "triglicer", "estatina")
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in termos), (
            f"{slug}: documento vinculado não menciona dislipidemia/colesterol/lipídios no texto"
        )
