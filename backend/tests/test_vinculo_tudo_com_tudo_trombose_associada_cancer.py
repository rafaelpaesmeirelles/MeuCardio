...
ficha já existente trombose-associada-cancer (área cardiooncologia) em
doencas/metadados.json.

Vigésimo nono lote de conteúdo do dia, terceiro desta natureza (após
doenca-de-kawasaki PR #642 e estenose-aortica-tavi-idoso PR #643): não é
aprofundamento de conteúdo clínico. A ficha já estava
completeness=completo, com todo o conteúdo clínico integral e
...
adiciona os 5 related_document_slugs e o patient_material_slug, ambos
verificados por leitura real dos documentos candidatos e do
material-paciente/metadados.json.

Nota de escopo: um candidato adicional
(rastreio-de-neoplasia-oculta-apos-tev-nao-provocado-os-ensaios-somit-e-
some) foi descartado por cobrir a relação inversa (TEV como achado que
leva à investigação de câncer oculto), fora do escopo desta ficha (TEV
EM paciente com câncer já conhecido).
...
SLUG = "trombose-associada-cancer"
...
def test_marcacao_editorial_correta():
    ...
    assert item.get("version") == 3, "fechamento de lacuna deveria incrementar version de 2 para 3"


def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Trombose associada ao câncer"
    assert "TEV no câncer" in (item.get("aliases") or [])
...
def test_related_document_slugs_mencionam_cancer_ou_oncologia():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona câncer/oncologia no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "cancer-e-trombose-por-que-o-anticoagulante-muda"
    materiais = {
        x["slug"]
