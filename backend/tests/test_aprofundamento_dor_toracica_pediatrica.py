...
hipertensao-arterial-pediatrica PR #609). Ficha tinha apenas metadados
de catalogação e 1 related_document_slug — dor torácica é a segunda
causa mais comum de encaminhamento à cardiologia pediátrica (atrás só
de sopro), embora causa cardíaca seja rara (1-4% dos casos).

O lote não cria nenhum documento, checklist, trilha ou material novo —
conecta a ficha a 13 itens já publicados e revisados em content/ (12
novos + 1 já existente), todos fora de Farmacologia/Calculadoras/
Exames.

Nota de compliance: este arquivo também espelha, em
test_disease_fragments_canonical.py, a mesma correção de allowlist já
aprovada pelo Rafael no PR #606, pois esta branch parte de origin/main
...
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # já existia no related_document_slugs original (1 item), antes deste
    # lote — também em sincope-pediatrica
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Dor torácica pediátrica"
    assert "dor no peito na criança" in (item.get("aliases") or [])
    assert item.get("category") == "sintoma_e_exame"
    assert item.get("subtype") == "dor_toracica"
    assert item.get("prevalence_rank") == 29
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 10, "hub deveria conectar um volume relevante de documentos"
...
def test_related_document_slugs_sao_todos_sobre_dor_toracica():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert "dor torácica" in texto or "dor toracica" in texto, (
            f"{slug}: documento vinculado não menciona dor torácica no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "dor-toracica-pediatrica"
    materiais = {
        x["slug"]
