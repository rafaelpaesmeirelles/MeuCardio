"""Contrato do verbete NOVO "cteph" (área geral, categoria circulacao_pulmonar)
criado em 29/08/2026 via doencas/fragmentos/cteph.json.

A ficha genérica já existente 'hipertensao-pulmonar' (revisada, publicada)
cobre os cinco grupos ESC/ERS 2022 em visão panorâmica e já menciona CTEPH
como grupo 4, mas sem a profundidade específica de fisiopatologia da
obstrução trombótica organizada, cintilografia V/Q versus angiotomografia,
critérios de operabilidade/decisão do heart team ou os dados quantitativos
de sobrevida operado-versus-não-operado — lacuna confirmada por leitura
completa do registro existente antes de criar este verbete.

Nota sobre verificação de citações: 7 PMIDs verificados individualmente via
NCBI e-utils (esummary) em 29/08/2026. Atenção especial ao MERIT-1
(macitentana): o registro original de 2017 (PMID 28919201) está oficialmente
marcado "Retracted Publication" no PubMed; a citação vigente usada nesta
ficha é a republicação de 2024 (PMID 38548406), com a nota de retratação
(PMID 38552648) também referenciada. A retratação decorreu de revisão de
integridade de dados hemodinâmicos numa fração da amostra (14/80
participantes), e uma análise de sensibilidade excluindo esses casos
confirmou o resultado positivo original — não houve reversão de conclusão
nem fraude. Ver review_note do próprio registro para o detalhamento
completo.
...
próprio registro e no relatório docs/novo-verbete-cteph-2026-08-29.md — não
...
checagem funciona corretamente — seguindo exatamente o padrão já usado pela
PR #698 (cardiomiopatia-de-takotsubo).

Nota sobre overlap com o hub 'hipertensao-pulmonar': os 6
related_document_slugs desta ficha já constam integralmente em
related_document_slugs de 'hipertensao-pulmonar'. Isso é overlap legítimo e
esperado — CTEPH é o grupo 4 dentro da classificação de cinco grupos que a
ficha genérica cobre de forma panorâmica — e está coberto por teste
dedicado abaixo.
"""
...
TERMOS_TEMA = ("cteph", "tromboembólica crônica", "tromboembolica cronica")
...
def test_catalogacao():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Hipertensão pulmonar tromboembólica crônica (CTEPH)"
    assert "CTEPH" in (item.get("aliases") or [])


def test_categoria_igual_a_do_hub_hipertensao_pulmonar():
    doencas = _load_doencas()
    assert doencas[SLUG]["category"] == doencas[HUB_SLUG]["category"]
...
def test_profundidade_minima_e_nao_e_resumo():
    ...
    # Os três eixos de profundidade exigidos pela missão: fisiopatologia/critérios
    # hemodinâmicos, cintilografia V/Q e decisão de operabilidade pelo heart team.
    assert "cintilografia_vq_versus_angiotomografia" in diagnostic
    assert "avaliacao_de_operabilidade_pelo_heart_team" in diagnostic


def test_texto_com_acentuacao_correta_do_portugues():
    item = _load_doencas()[SLUG]
    serialized = json.dumps(item, ensure_ascii=False)
    for palavra in ("não ", "cirúrgica", "hipertensão", "cardíaco", "átrio"):
...
def test_nenhuma_dose_de_farmaco_em_nenhum_campo():
    ...
    # Riociguate e macitentana só podem aparecer como classe terapêutica, nunca com dose.
    assert "riociguate" in serialized.lower() or "riociguat" in serialized.lower()
    assert "macitentana" in serialized.lower()
...
def test_related_document_slugs_mencionam_tema_de_forma_central():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert ocorrencias >= 3, (
            f"{slug}: documento vinculado menciona CTEPH só {ocorrencias}x — "
            "não parece discussão central do tema"
        )


def test_sobreposicao_com_hub_hipertensao_pulmonar_e_explicitamente_documentada():
    doencas = _load_doencas()
    item = doencas[SLUG]
    related = set(item.get("related_document_slugs") or [])
    hub = doencas[HUB_SLUG]
    hub_related = set(hub.get("related_document_slugs") or [])

    # Os 6 documentos desta ficha são um subconjunto do hub genérico — CTEPH é
    # o grupo 4 dentro da classificação de cinco grupos que o hub cobre.
    assert related <= hub_related, "related_document_slugs desta ficha deveria ser subconjunto do hub 'hipertensao-pulmonar'"
    ...
    assert compartilhados_com_outras_fichas == set(), (
        "sobreposição não documentada fora do hub hipertensao-pulmonar: "
...
def test_patient_material_slug_resolve():
    ...
    assert material == "cteph-hipertensao-pulmonar-por-coagulos-antigos"
    materiais = {
        x["slug"]
...
def test_slug_nao_colide_com_hub_hipertensao_pulmonar():
    assert SLUG != HUB_SLUG


def test_merit1_situacao_de_retratacao_documentada():
    item = _load_doencas()[SLUG]
    refs = " | ".join(item.get("source_refs") or [])
    # A citação vigente é a republicação de 2024, não o registro retratado de 2017.
    assert "38548406" in refs
    assert "38552648" in refs
    assert "28919201" in refs and "RETRAT" in refs.upper()
    # A situação também deve estar documentada em special_populations, não só no source_ref.
    populations = " | ".join(item.get("special_populations") or [])
    assert "retrat" in populations.lower() or "republicad" in populations.lower()
