"""Contrato do verbete NOVO "protese-valvar-mecanica" (área geral) criado em
29/08/2026 via doencas/fragmentos/protese-valvar-mecanica.json — a entidade
"prótese valvar mecânica" (escolha, anticoagulação e trombose) não tinha
ficha própria em doencas/metadados.json, apesar de corpus rico já existente
em content/Valvopatias/ (escolha mecânica vs. biológica, alvo de INR,
trombose aguda e fluxogramas ESC/EACTS 2021/2025) e content/Tromboembolismo/
(RE-ALIGN sobre contraindicação de DOAC, e revisão histórica PRO-TEE/
Roudaut/Karthikeyan sobre trombólise vs. cirurgia).

Nota sobre verificação de citações: os 9 PMIDs desta rodada foram
verificados individualmente via NCBI e-utils (esummary) antes da montagem
do source_refs; todos batem exatamente com a citação já usada nos
documentos-fonte publicados desta biblioteca, sem necessidade de correção.
...
docs/novo-verbete-protese-valvar-mecanica-2026-08-29.md — não é bug e não
...
Nota sobre category: 'valvopatia_e_anticoagulacao' reutiliza a única
convenção já existente no sistema para esse cruzamento temático (usada uma
vez, em 'protese-mecanica-na-gravidez'), confirmada por listagem
programática de todas as categorias em doencas/metadados.json e
doencas/fragmentos/*.json.

Nota sobre sobreposição de related_document_slugs: ao contrário do padrão
"um hub só" usado em cardiomiopatia-de-takotsubo, este verbete compartilha
slugs com TRÊS registros já existentes — o hub geral 'valvopatias' (5 dos 7
slugs), 'protese-mecanica-na-gravidez' (2 slugs) e 'anticoagulacao-idoso' (1
slug) — porque são recortes por população (gestação) ou faixa etária (idoso)
do mesmo corpo de evidência sobre INR-alvo e contraindicação de DOAC, não
fichas concorrentes sobre o mesmo escopo. A sobreposição é documentada
explicitamente e testada abaixo.
"""
...
TERMOS_TEMA = ("prótese", "protese")

EXPECTED_OVERLAPS = {
    "valvopatias": {
        "trombose-de-protese-valvar-mecanica-diagnostico-e-decisao-entre-fibrinolise-e-cirurgia",
        "fluxograma-trombose-de-protese-valvar-mecanica-fibrinolise-vs-cirurgia-esc-eacts-2021",
        "fluxograma-trombose-de-protese-valvar-mecanica-fibrinolise-vs-cirurgia-esc-eacts-2025",
        "protese-valvar-escolha-mecanica-vs-biologica-e-alvo-de-inr-esc-eacts-2025",
        "fluxograma-escolha-de-protese-valvar-mecanica-vs-biologica-esc-eacts-2025",
    },
    "protese-mecanica-na-gravidez": {
        "anticoagulante-oral-direto-em-protese-valvar-mecanica-por-que-e-contraindicado-re-align",
        "protese-valvar-escolha-mecanica-vs-biologica-e-alvo-de-inr-esc-eacts-2025",
    },
...
def test_marcacao_editorial_correta():
    ...
    assert item.get("category") == "valvopatia_e_anticoagulacao"
    assert item.get("subtype") == "protese_mecanica"
...
def test_catalogacao():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Prótese valvar mecânica"
    assert "Mechanical heart valve (MHV)" in (item.get("aliases") or [])


def test_categoria_reutiliza_convencao_ja_existente():
    doencas = _load_doencas()
    categorias = {slug: item.get("category") for slug, item in doencas.items()}
    assert categorias.get("protese-mecanica-na-gravidez") == "valvopatia_e_anticoagulacao"
    assert categorias[SLUG] == "valvopatia_e_anticoagulacao"
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "coração", "válvula", "anticoagulação", "cirúrgico"):
...
def test_assistente_deterministico_seguro():
    ...
    assert len(rule_ids) == len(set(rule_ids)), "ids de regra repetidos"
...
def test_related_document_slugs_mencionam_tema_de_forma_central():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert ocorrencias >= 5, (
            f"{slug}: documento vinculado menciona 'prótese' só {ocorrencias}x — "
            "não parece discussão central do tema"
        )


def test_biopotese_nao_entrou_no_vinculo_por_ser_fora_de_escopo():
    item = _load_doencas()[SLUG]
    related = set(item.get("related_document_slugs") or [])
    assert "antitrombotico-nos-primeiros-3-meses-apos-biopotese-valvar-esc-eacts-2021" not in related


def test_sobreposicao_com_registros_existentes_e_explicitamente_documentada():
    doencas = _load_doencas()
    item = doencas[SLUG]
    related = set(item.get("related_document_slugs") or [])

    for outro_slug, esperado in EXPECTED_OVERLAPS.items():
    ...
    # Nenhuma sobreposição não documentada com qualquer outro registro do catálogo.
    esperados_totais: set[str] = set()
    for valores in EXPECTED_OVERLAPS.values():
        esperados_totais |= valores

    for outro_slug, outro_item in doencas.items():
        if outro_slug in {SLUG, *EXPECTED_OVERLAPS.keys()}:
            continue
        compartilhados = related & set(outro_item.get("related_document_slugs") or [])
        assert compartilhados == set(), (
            f"sobreposição não documentada com {outro_slug}: {compartilhados}"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "vivendo-com-uma-valvula-mecanica-anticoagulacao-e-cuidados-no-dia-a-dia"
    materiais = {
        x["slug"]
...
def test_slug_nao_colide_com_verbete_hub_valvopatias_geral():
    assert SLUG != "valvopatias"


def test_pmids_citados_estao_presentes_no_source_ref():
    item = _load_doencas()[SLUG]
    refs = " | ".join(item.get("source_refs") or [])
    for pmid in (
        "40878295", "34453165", "40574603", "19427604", "23991661",
        "14715187", "23329151", "17170355", "42412081",
    ):
