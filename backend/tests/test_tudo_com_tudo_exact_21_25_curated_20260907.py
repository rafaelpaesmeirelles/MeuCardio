...
FIXTURE = ROOT / "scripts/fixtures/tudo_com_tudo_exact_21_25_curated_20260907.json"
...
def test_fixture_exact_21_25_tem_92_relacoes_em_4_doencas():
    rows = _fixture()
    assert len(rows) == 92
    assert len({row["source_disease_slug"] for row in rows}) == 4


def test_fixture_exact_21_25_esta_persistida_e_revisada():
...
def test_fixture_exact_21_25_respeita_politica_clinica():
    for item in _fixture():
        validar_relacao_clinica(
            ...
            evidence_source="scripts/fixtures/tudo_com_tudo_exact_21_25_curated_20260907.json",
            extra={"review_note": "coorte exact 21-25 curada"},
        )


def test_manifesto_exact_21_25_sem_duplicatas():
    relations = _relations()
    keys = [
