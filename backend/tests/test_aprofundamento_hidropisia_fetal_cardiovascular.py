...
já existente hidropisia-fetal-cardiovascular (área cardiopediatria) em
...
ecocardiograma-fetal PR #630, bloqueio-atrioventricular-fetal
PR #631). Ficha já tinha patient_material_slug e 2
...
conecta a ficha a 5 itens já publicados e revisados em content/, todos
fora de Farmacologia/Calculadoras/Exames.

Nota sobre sobreposição: 3 dos related_document_slugs são
compartilhados com fichas irmãs do cluster fetal (uma já aprofundada
hoje — bloqueio-atrioventricular-fetal/PR #631 —, outras ainda não —
taquicardia-supraventricular-fetal, sindrome-coracao-esquerdo-
hipoplasico-fetal, anomalia-ebstein, flutter-atrial-fetal,
tumores-cardiacos-fetais) — overlap pré-existente e/ou legítimo, não
introduzido por este lote.
...
SLUG = "hidropisia-fetal-cardiovascular"
...
TERMOS_TEMA = ("hidropisia", "hidrópsia", "hydrops", "síndrome em espelho")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em taquicardia-supraventricular-fetal, flutter-atrial-fetal
    # e bloqueio-atrioventricular-fetal (PR #631)
    "arritmias-fetais-taquicardia-supraventricular-e-bloqueio-atrioventricular-total-tratamento-transplacentario",
    # também em sindrome-coracao-esquerdo-hipoplasico-fetal
    "intervencao-cardiaca-fetal-valvuloplastia-aortica-fetal-e-feto-hidropico-registro-internacional",
    # também em bloqueio-atrioventricular-fetal (PR #631)
    "bloqueio-av-congenito-completo-no-recem-nascido",
    # também em tumores-cardiacos-fetais
    "tumores-cardiacos-fetais-rabdomioma-e-diagnostico-diferencial-de-massa-cardiaca-intrautero",
    # também em anomalia-ebstein
    "anomalia-de-ebstein-neonatal-estratificacao-de-risco-e-manejo-do-shunt-circular",
}
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Hidropisia fetal de possível causa cardiovascular"
    assert "hydrops fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "insuficiencia_cardiaca_fetal"
    assert item.get("prevalence_rank") == 36
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "hidropisia"):
...
def test_related_document_slugs_mencionam_hidropisia():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona hidropisia no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "hidropisia-fetal-cardiovascular"
    materiais = {
        x["slug"]
