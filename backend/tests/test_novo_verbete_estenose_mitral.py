"""Contrato do verbete NOVO "estenose-mitral" (área geral) criado em
29/08/2026 via doencas/fragmentos/estenose-mitral.json — valvopatia mais
associada à febre reumática e causa importante de morbimortalidade em
países de baixa/média renda, sem ficha própria em doencas/metadados.json
apesar de corpus rico já existente (fluxograma ESC/EACTS 2025, rastreamento
WHF/valvuloplastia, INVICTUS, formas na gestação/idoso/degenerativa).

Nota: todos os 6 PMIDs desta rodada verificados individualmente via NCBI
...
TERMOS_TEMA = ("estenose mitral", "estenose valvar mitral", "estenose da valva mitral", "valvotomia mitral")
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    "estenose-mitral-diagnostico-e-manejo-esc-eacts-2025",  # também em valvopatias (hub geral)
    "fluxograma-estenose-mitral-grave-comissurotomia-vs-cirurgia-esc-eacts-2025",  # também em valvopatias
    "doenca-valvar-mitral-degenerativa-no-idoso-calcificacao-anular-e-opcoes-percutaneas",  # também em valvopatias
    "estenose-mitral-descompensada-na-gestacao-e-edema-pulmonar",  # também em valvopatias-na-gravidez
}
...
def test_catalogacao():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Estenose mitral"
    assert "Doença mitral reumática" in (item.get("aliases") or [])
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "válvula" if "válvula" in serialized else "gestação", "área"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(t in texto for t in TERMOS_TEMA), f"{slug}: não menciona estenose mitral"
...
def test_patient_material_slug_resolve():
    ...
    assert material == "estenose-mitral-entendendo-a-valvula-estreitada"
    materiais = {
        x["slug"]
