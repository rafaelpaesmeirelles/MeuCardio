...
já existente cardiotoxicidade-bcr-abl (área cardiooncologia) em
...
gravidez PR #616, hipertensao-pulmonar-gravidez PR #621). Ficha tinha
apenas metadados de catalogação — zero campos clínicos, zero
...
TERMOS_TEMA = ("bcr-abl", "bcr abl", "imatinib", "dasatinib", "nilotinib", "bosutinib", "ponatinib")
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Toxicidade cardiovascular por inibidores BCR-ABL"
    assert "TKI BCR-ABL e coração" in (item.get("aliases") or [])
    assert item.get("category") == "terapia_alvo"
    assert item.get("subtype") == "bcr_abl"
    assert item.get("prevalence_rank") == 7
...
def test_related_document_slugs_sao_todos_sobre_bcr_abl():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona BCR-ABL nem os fármacos "
            "(imatinibe, dasatinibe, nilotinibe, bosutinibe, ponatinibe) no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "cardiotoxicidade-bcr-abl"
    materiais = {
        x["slug"]
