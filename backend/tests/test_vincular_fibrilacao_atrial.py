...
— ficha já completa `fibrilacao-atrial` (área geral, arritmia mais
prevalente da prática clínica) em doencas/metadados.json, que atendia o
piso técnico mínimo (3 related_document_slugs) mas era desproporcionalmente
rasa frente ao volume de corpus disponível (~59 documentos mencionam FA).
...
TERMOS_TEMA = ("fibrilação atrial", "fibrilacao atrial")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em fibrilacao-atrial-no-idoso
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Fibrilação atrial"
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert "cha2ds2-va" not in related, "candidato de calculadora não deveria ter sido incluído"
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona fibrilação atrial no texto"
        )
