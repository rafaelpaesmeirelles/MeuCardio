...
FIXTURE = ROOT / "scripts/fixtures/tudo_com_tudo_exact_6_7_candidates_20260907.json"
...
def test_fixture_exact_6_7_tem_45_relacoes_em_7_doencas():
    rows = _fixture()
    assert len(rows) == 45
    assert len({row["source_disease_slug"] for row in rows}) == 7


def test_fixture_exact_6_7_esta_persistida_e_revisada():
...
def test_fixture_exact_6_7_respeita_politica_clinica():
    for item in _fixture():
        validar_relacao_clinica(
            ...
            evidence_source="scripts/fixtures/tudo_com_tudo_exact_6_7_candidates_20260907.json",
            extra={"review_note": "coorte exact 6-7 revisada"},
        )


def test_manifesto_exact_6_7_sem_duplicatas():
    relations = _relations()
    keys = [
        (
