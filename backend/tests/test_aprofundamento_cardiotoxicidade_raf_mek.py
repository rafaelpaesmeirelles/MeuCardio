...
já existente cardiotoxicidade-raf-mek (área cardiooncologia) em
doencas/metadados.json.

Trigésimo oitavo lote de conteúdo do dia. Ficha estava
completeness=basico, só catalogação (1 source_ref: ESC 2022), zero
campos clínicos, mas já tinha 1 related_document_slug e
patient_material_slug preenchidos.
...
escopo real da PR #551 mudou (não toca mais este slug), liberando-a.

Nota sobre verificação de citações: todos os 6 PMIDs de epidemiologia
mais 1 PMID citado em special_populations (uso pediátrico off-label na
síndrome de Noonan) foram verificados individualmente via NCBI e-utils
antes da montagem — nenhuma correção foi necessária.

Nota sobre o vínculo pré-existente: um agente de pesquisa confundiu o
related_document_slug pré-existente (documento narrativo real e
distinto) com o slug da própria ficha — verifiquei diretamente que são
strings diferentes ("cardiotoxicidade-raf-mek" é o slug da ficha;
"toxicidade-cardiovascular-dos-inibidores-de-braf-e-mek-disfuncao-
ventricular-hipertensao-e-vigilancia" é o documento real, existente e
central) e mantive o vínculo.
"""
...
TERMOS_TEMA = ("raf", "mek", "braf", "trametinibe", "vemurafenibe", "dabrafenibe")
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Toxicidade cardiovascular por inibidores RAF/MEK"
    assert "MEK e coração" in (item.get("aliases") or [])
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona RAF/MEK/BRAF no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "cardiotoxicidade-raf-mek"
    materiais = {
        x["slug"]
