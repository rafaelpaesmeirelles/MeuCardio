...
já existente seguimento-cardiovascular-pos-parto (área gravidez) em
doencas/metadados.json.

Décimo sexto e último lote de conteúdo do dia (após doenca-coronariana-
...
Ficha tinha apenas metadados de catalogação — zero campos clínicos,
zero related_document_slugs, sem patient_material_slug. Fecha o
cluster de gravidez pós-parto do dia (irmã de
medicamentos-cardiovasculares-gestacao-lactacao/PR #625 e
plano-parto-cardiopatia-materna/PR #626, mesmo nível de raspagem
original).
...
SLUG = "seguimento-cardiovascular-pos-parto"
...
TERMOS_TEMA = (
    "pós-parto", "pos-parto", "puerpério", "puerperio",
    "risco cardiovascular", "seguimento", "longo prazo",
)

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em pre-eclampsia-e-risco-cardiovascular
    "hipertensao-gestacional-e-pre-eclampsia-risco-cardiovascular-materno-de-longo-prazo",
    # também em cardiomiopatia-periparto
    "cardiomiopatia-periparto-criterios-diagnosticos-recuperacao-e-manejo",
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Seguimento cardiovascular pós-parto"
    assert "cardiologia no puerpério" in (item.get("aliases") or [])
    assert item.get("category") == "seguimento"
    assert item.get("subtype") == "pos_parto"
    assert item.get("prevalence_rank") == 16
...
def test_texto_com_acentuacao_correta_do_portugues():
    """Lote anterior (plano-parto-cardiopatia-materna) recebeu texto sem
    diacríticos de um dos agentes de pesquisa — checagem preventiva de
    que palavras comuns aparecem acentuadas no texto final."""
    item = _load_doencas()[SLUG]
    serialized = json.dumps(item, ensure_ascii=False)
    for palavra in ("não ", "cardíaca", "gestação", "pós-parto", "pré-eclâmpsia"):
...
def test_related_document_slugs_mencionam_seguimento_pos_parto():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona pós-parto/puerpério/"
            "risco cardiovascular de longo prazo no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "cuidado-do-coracao-no-primeiro-ano-depois-do-parto"
    materiais = {
        x["slug"]
