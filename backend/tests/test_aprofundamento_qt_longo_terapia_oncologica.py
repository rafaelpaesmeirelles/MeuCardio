...
já existente qt-longo-terapia-oncologica (área cardiooncologia) em
doencas/metadados.json.

Trigésimo nono lote de conteúdo do dia. Ficha estava
completeness=basico, só catalogação (1 source_ref: ESC 2022), zero
campos clínicos. Terceira ficha desbloqueada hoje pela reavaliação de
escopo da PR #551.
...
TERMOS_TEMA = ("qt", "torsades", "qtc")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em cardiotoxicidade-bcr-abl e cardiotoxicidade-raf-mek
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Prolongamento do QT em terapia oncológica"
    assert "QT longo por quimioterapia" in (item.get("aliases") or [])
    assert item.get("category") == "arritmia"
    assert item.get("prevalence_rank") == 12
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "prolongamento", "monitorização"):
...
def test_patient_material_slug_resolve():
    ...
    assert material == "qt-longo-terapia-oncologica"
    materiais = {
        x["slug"]
