...
já existente cardiopatia-congenita-gravidez (área gravidez) em
doencas/metadados.json.

Décimo primeiro lote de aprofundamento do dia (após doenca-coronariana-
...
cuidados-paliativos-cardiovasculares PR #615). Ficha tinha apenas
...
Nota de compliance: o texto de special_populations gerado pelo agente
de pesquisa usava a sigla "mWHO" (mWHO IV, mWHO 2.0) para a
classificação de risco materno da OMS/ESC — reescrito por extenso na
montagem para não conter a substring banida "mwho", mesmo precedente
já aplicado em cardiopatia-congenita-do-adulto/PR #596. O único uso
remanescente de "mwho" é dentro do slug de um documento vinculado
(sindrome-de-eisenmenger-contraindicacao-absoluta-a-gestacao-mecanismo-
mwho-iv), não em texto autoral — mesmo padrão já usado para
"classificacao-de-risco-mwho-2-0-na-gravidez-esc-2025" no lote de
...
SLUG = "cardiopatia-congenita-gravidez"
...
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    ...
    # também em tetralogia-de-fallot e indicacoes-ecocardiograma-fetal
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Cardiopatia congênita na gravidez"
    assert "GUCH e gestação" in (item.get("aliases") or [])
    assert item.get("category") == "cardiopatia_congenita"
    assert item.get("subtype") == "gestacao"
    assert item.get("prevalence_rank") == 10
...
def test_related_document_slugs_sao_todos_sobre_cardiopatia_congenita():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        texto = documentos[slug].read_text(encoding="utf-8", errors="replace").casefold()
        assert "cardiopatia congênita" in texto or "cardiopatia congenita" in texto, (
            f"{slug}: documento vinculado não menciona cardiopatia congênita no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "gravidez-e-cardiopatia-congenita-o-que-saber"
    materiais = {
        x["slug"]
