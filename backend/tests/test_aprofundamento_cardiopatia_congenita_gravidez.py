...
DOSE_PATTERNS = (r"\d+[\.,]?\d*\s*mg(?!/d[lL])\b",r"\d+[\.,]?\d*\s*mg/kg",r"\d+[\.,]?\d*\s*mcg")
...
def test_related_document_slugs_sao_todos_sobre_cardiopatia_congenita():
    ...
    for slug in item.get("related_document_slugs") or []:
        texto=documentos[slug].read_text(encoding="utf-8",errors="replace").casefold(); assert "cardiopatia congênita" in texto or "cardiopatia congenita" in texto
...
def test_patient_material_slug_resolve():
    item=_load_doencas()[SLUG]; material=item.get("patient_material_slug"); assert material=="gravidez-e-cardiopatia-congenita-o-que-saber"; materiais={x["slug"] for x in json.loads((REPOSITORY_ROOT/"material-paciente/metadados.json").read_text(encoding="utf-8"))}; assert material in materiais
