...
já existente flutter-atrial-fetal (área cardiopediatria) em
doencas/metadados.json.

Vigésimo lote de conteúdo do dia, quarto do cluster de cardiologia
fetal (após doenca-coronariana-idoso PR #603, valva-aortica-bicuspide-
pediatrica PR #604, hipotensao-ortostatica-no-idoso PR #606,
sopros-na-infancia PR #608, hipertensao-arterial-pediatrica PR #609,
dor-toracica-pediatrica PR #610, dislipidemias-pediatricas PR #611,
arritmias-pediatricas PR #612, avaliacao-multidimensional-
...
PR #632). Ficha já tinha patient_material_slug e 1
...
Nota sobre correção de citação: durante a verificação de PMIDs deste
lote, foi descoberto que a referência de Krapp et al. usada no lote
anterior (hidropisia-fetal-cardiovascular/PR #632) tinha um PMID
incorreto (12860870, pertencente a outro artigo) — corrigido
separadamente naquele PR (commit próprio, verificado via NCBI
e-utils). O PMID correto (12860871) é usado diretamente aqui.

Nota sobre sobreposição: 3 dos related_document_slugs são
compartilhados com fichas irmãs do cluster fetal já aprofundadas hoje
(taquicardia-supraventricular-fetal, bloqueio-atrioventricular-fetal/
PR #631, hidropisia-fetal-cardiovascular/PR #632).
...
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em taquicardia-supraventricular-fetal, bloqueio-
    # atrioventricular-fetal (PR #631) e hidropisia-fetal-
    # cardiovascular (PR #632)
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Flutter atrial fetal"
    assert "flutter fetal" in (item.get("aliases") or [])
    assert item.get("category") == "cardiologia_fetal"
    assert item.get("subtype") == "taquiarritmia"
    assert item.get("prevalence_rank") == 33
...
def test_related_document_slugs_mencionam_flutter_atrial():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona flutter atrial no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "flutter-atrial-fetal"
    materiais = {
        x["slug"]
