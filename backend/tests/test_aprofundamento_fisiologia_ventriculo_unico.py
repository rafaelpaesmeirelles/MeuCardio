...
já existente fisiologia-ventriculo-unico (área cardiopediatria) em
doencas/metadados.json.

Vigésimo quinto lote de conteúdo do dia. Ficha já tinha
patient_material_slug e 3 related_document_slugs preenchidos, mas zero
campos clínicos (completeness=basico, 1 source_ref).
...
fora de Farmacologia/Calculadoras/Exames (3 originais + 4 novos).
...
correção de PMID foi necessária, mas listas de autores não confirmadas
via e-utils foram escritas como "Autor Principal, et al." em vez de
fabricar nomes não verificados.

Nota sobre sobreposição: overlap parcial e pré-existente (não introduzido
por este lote) com transposicao-das-grandes-arterias (compartilha
circulacao-de-fontan-e-transposicao-das-grandes-arterias-no-adulto) e com
atresia-tricuspide (compartilha ventriculo-unico-e-estadiamento-de-fontan-
fisiologia-cirurgica-em-tres-estagios-e-complicacoes) — documentado abaixo.
...
SLUG = "fisiologia-ventriculo-unico"
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Fisiologia de ventrículo único"
    assert "circulação de Fontan" in (item.get("aliases") or [])
    assert item.get("category") == "cardiopatia_congenita"
    assert item.get("subtype") == "coracao_univentricular"
    assert item.get("prevalence_rank") == 21
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "circulação", "função"):
...
def test_related_document_slugs_mencionam_ventriculo_unico_ou_fontan():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona ventrículo único/Fontan no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "fisiologia-ventriculo-unico"
    materiais = {
        x["slug"]
