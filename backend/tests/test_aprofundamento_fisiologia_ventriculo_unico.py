...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ","cardíaca","circulação","função"): assert palavra in serialized
...
def test_special_populations_nao_contem_substring_banida():
    serialized=json.dumps(_load_doencas()[SLUG].get("special_populations"),ensure_ascii=False).casefold(); assert "mwho" not in serialized; assert "hfa-icos" not in serialized
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    item=_load_doencas()[SLUG]; documentos=_all_document_paths(); related=item.get("related_document_slugs") or []; assert 3<=len(related)<=7; assert [slug for slug in related if slug not in documentos]==[]; assert [slug for slug in related if any(pasta in str(documentos[slug]) for pasta in PASTAS_NAO_DOCUMENTO)]==[]; assert len(related)==len(set(related))
def test_related_document_slugs_mencionam_ventriculo_unico_ou_fontan():
    ...
    for slug in item.get("related_document_slugs") or []:
        texto=documentos[slug].read_text(encoding="utf-8",errors="replace").casefold(); assert any(termo in texto for termo in TERMOS_TEMA)
...
def test_patient_material_slug_resolve():
    item=_load_doencas()[SLUG]; material=item.get("patient_material_slug"); assert material=="fisiologia-ventriculo-unico"; materiais={x["slug"] for x in json.loads((REPOSITORY_ROOT/"material-paciente/metadados.json").read_text(encoding="utf-8"))}; assert material in materiais