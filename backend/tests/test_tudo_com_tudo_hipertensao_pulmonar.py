...
Hipertensão Pulmonar adulta (área geral) em doencas/metadados.json.

Terceiro ciclo independente do dia (após endocardite infecciosa, PR #553,
e pericardite, PR #554). A coleção já tinha hipertensao-pulmonar-pediatrica
e hipertensao-pulmonar-gravidez (escopos distintos), mas nenhum hub geral
de adulto. O lote não cria nenhum documento, checklist, trilha ou material
novo — conecta o novo verbete a 60 itens de content/Hipertensão_pulmonar/
já publicados e revisados (o maior lote de conexão dos três ciclos).
"""
...
NOVO_SLUG = "hipertensao-pulmonar"
...
def test_slug_e_genuinamente_novo():
    ...
    assert "hipertensao-pulmonar-pediatrica" in doencas
    assert "hipertensao-pulmonar-gravidez" in doencas
    ped = doencas["hipertensao-pulmonar-pediatrica"]
    grav = doencas["hipertensao-pulmonar-gravidez"]
    assert ped["area"] == "cardiopediatria"
    assert grav["area"] == "gravidez"
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 30, "hub de HP deveria conectar um volume grande de documentos (a maior pasta entre os ciclos)"
...
def test_related_document_slugs_sao_todos_sobre_hipertensao_pulmonar():
    """Vínculo direto: todo documento conectado deve mencionar hipertensão
    pulmonar explicitamente no próprio texto — evita link por proximidade
    temática."""
    item = _load_doencas()[NOVO_SLUG]
    documentos = _all_document_paths()
    for slug in item.get("related_document_slugs") or []:
        texto = documentos[slug].read_text(encoding="utf-8").casefold()
        assert "pulmonar" in texto, f"{slug}: documento vinculado não menciona (hipertensão) pulmonar no texto"


def test_nao_duplica_conteudo_de_gravidez_ou_pediatria():
    """O documento específico de HAP na gestação não deve ser vínculo direto
    do verbete geral — pertence ao registro hipertensao-pulmonar-gravidez."""
    item = _load_doencas()[NOVO_SLUG]
    related = set(item.get("related_document_slugs") or [])
    assert "hipertensao-arterial-pulmonar-na-gestacao-risco-manejo-e-desfechos" not in related
