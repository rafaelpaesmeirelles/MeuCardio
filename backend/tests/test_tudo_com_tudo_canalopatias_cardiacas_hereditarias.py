...
Canalopatias cardíacas hereditárias (área geral) em doencas/metadados.json.

Décimo quarto ciclo independente do dia (após endocardite infecciosa
...
aorta PR #580 e taquicardia supraventricular PR #581). Não havia hub geral
de canalopatias cardíacas hereditárias do adulto — apenas o hub-irmão
canalopatias-pediatricas (área cardiopediatria) e qt-longo-terapia-oncologica
(forma ADQUIRIDA por quimioterapia, entidade distinta). O lote não cria
nenhum documento, checklist, trilha ou material novo — conecta o novo
verbete a 19 itens já publicados e revisados em content/, todos fora de
Farmacologia/Calculadoras/Exames.

Nota: um documento (canalopatias-sindrome-do-qt-longo-e-sindrome-de-brugada-
diagnostico-e-manejo) está deliberadamente vinculado também ao hub
canalopatias-pediatricas — inclusão dupla legítima e documentada, análoga
ao caso do documento VIBORG compartilhado entre os hubs de aorta e DAP de
membros em ciclo anterior. Os outros 5 documentos pediátricos-específicos
do hub-irmão NÃO foram duplicados — verificação dedicada abaixo.
"""
...
NOVO_SLUG = "canalopatias-cardiacas-hereditarias"
...
SLUGS_IRMAOS_NAO_DUPLICAR = {
    ...
    "qt-longo-terapia-oncologica",
}

DOCUMENTOS_PEDIATRICOS_ESPECIFICOS_NAO_DUPLICAR = {
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 12, "hub de canalopatias deveria conectar um volume relevante de documentos"
...
def test_nao_duplica_documentos_pediatricos_especificos_do_hub_irmao():
    item = _load_doencas()[NOVO_SLUG]
    related = set(item.get("related_document_slugs") or [])
    overlap = related & DOCUMENTOS_PEDIATRICOS_ESPECIFICOS_NAO_DUPLICAR
    assert overlap == set(), (
        f"related_document_slugs duplica documento pediátrico-específico do hub-irmão canalopatias-pediatricas: {overlap}"
    )


def test_related_document_slugs_sao_todos_sobre_canalopatias():
    """Vínculo direto: todo documento conectado deve mencionar canalopatia/
    QT longo/Brugada/CPVT explicitamente no próprio texto — evita link por
    proximidade temática."""
    item = _load_doencas()[NOVO_SLUG]
    documentos = _all_document_paths()
    termos = ("canalopatia", "qt longo", "qt curto", "brugada", "cpvt", "andersen-tawil", "fibrilação ventricular idiopática", "catecolaminérgica")
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in termos), (
            f"{slug}: documento vinculado não menciona canalopatia/QT longo/Brugada/CPVT no texto"
        )
