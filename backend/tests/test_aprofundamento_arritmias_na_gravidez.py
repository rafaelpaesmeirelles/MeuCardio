...
existente arritmias-na-gravidez (área gravidez) em doencas/metadados.json.

Ficha estava completeness=intermediario, zero related_document_slugs, zero
...
Nota sobre related_document_slugs: os 7 vínculos foram encontrados do zero
e cada um verificado por leitura direta do documento completo antes da
montagem.
"""
...
TERMOS_TEMA = ("gestaç", "gestant", "gravidez", "puerpér", "arritmia")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em medicamentos-cardiovasculares-gestacao-lactacao
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Arritmias na gravidez"
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "gestação", "arritmia"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona gestação/arritmia no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "arritmia-na-gravidez-remedios-seguros-para-o-coracao"
    materiais = {
        x["slug"]
