...
FIXTURE = ROOT / "scripts/fixtures/tudo_com_tudo_exact_16_20_curated_20260907.json"
...
def test_fixture_exact_16_20_tem_36_relacoes_em_2_doencas():
    rows = _fixture()
    assert len(rows) == 36
    assert len({row["source_disease_slug"] for row in rows}) == 2


def test_fixture_exact_16_20_esta_persistida_e_revisada():
...
def test_contextos_especiais_nao_superafirmam_supported_by():
    rows = _fixture()
    for item in rows:
        if item["source_disease_slug"] == "disfuncao-do-no-sinusal" and item["target_slug"].startswith("chagas-"):
            assert item["relation_type"] == "associated_with"
        if item["source_disease_slug"] == "taquicardia-supraventricular" and item["target_slug"].startswith("gravidez-"):
            assert item["relation_type"] == "associated_with"


def test_fixture_exact_16_20_respeita_politica_clinica():
    for item in _fixture():
        validar_relacao_clinica(
            ...
            evidence_source="scripts/fixtures/tudo_com_tudo_exact_16_20_curated_20260907.json",
            extra={"review_note": "coorte exact 16-20 curada"},
        )


def test_manifesto_exact_16_20_sem_duplicatas():
    relations = _relations()
    keys = [
