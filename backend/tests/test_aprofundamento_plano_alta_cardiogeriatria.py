...
existente plano-alta-cardiogeriatria (área cardiogeriatria) em
doencas/metadados.json.

Quadragésimo lote de conteúdo do dia. Ficha estava completeness=basico, só
catalogação (1 source_ref: AHA Scientific Statement), zero campos clínicos,
mas já tinha 3 related_document_slugs pré-existentes (piso mínimo já
atendido) e review_status=revisado (revertido para pendente_revisao porque
todo o conteúdo clínico novo ainda não passou por revisão humana). Quarta
...
Nota sobre related_document_slugs: os 3 vínculos pré-existentes foram
reconfirmados por leitura direta dos documentos (PACT-HF, telessaúde/
monitoramento remoto na IC do idoso, reabilitação cardíaca no muito idoso).
Um grep amplo em content/Cardiologia_geriátrica não encontrou nenhum
candidato adicional genuinamente central ao tema de plano de alta — o total
permanece em 3 (piso mínimo da regra Tudo com Tudo), sem fabricar vínculos.
patient_material_slug permanece vazio: nenhuma correspondência inequívoca
encontrada em material-paciente/metadados.json.
"""
...
SLUG = "plano-alta-cardiogeriatria"
...
TERMOS_TEMA = ("alta", "readmiss", "reinternaç", "transição de cuidado", "transicao")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em fragilidade-pre-procedimento-cardiovascular e doenca-coronariana-idoso
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Plano de alta cardiogeriátrico"
    assert "alta segura do idoso cardiopata" in (item.get("aliases") or [])
    assert item.get("category") == "transicao_cuidado"
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "reconciliação", "readmissão"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona alta/readmissão/transição de cuidado no texto"
        )
