...
Miocardite (área geral) em doencas/metadados.json.

Sétimo ciclo independente do dia (após endocardite infecciosa PR #553,
pericardite PR #554, hipertensão pulmonar PR #555, síncope PR #560,
valvopatias PR #563 e cardiomiopatias PR #565). Não havia nenhum verbete-hub
geral de miocardite do adulto — só entries de subpopulação
(miocardite-pediatrica, miocardite-por-inibidor-checkpoint). O lote não cria
nenhum documento, checklist, trilha ou material novo — conecta o novo
verbete a 12 documentos já publicados e revisados, cada um com menção
central e explícita ao tema, após varredura exaustiva de 123 ocorrências de
"miocardite/miopericardite/inflamação miocárdica" em content/. Antes do
commit foi feita checagem de coordenação com o trabalho concorrente do
Codex/ChatGPT (13 PRs abertos codex/* inspecionados via gh pr diff): nenhum
cria ou edita um slug geral de miocardite em doencas/metadados.json.
"""
...
def test_vinculos_tudo_com_tudo_resolvem_e_sao_documentos_narrativos():
    ...
    assert len(related) >= 8, "hub de miocardite deveria conectar um volume relevante de documentos"
...
def test_related_document_slugs_sao_todos_sobre_miocardite():
    """Vínculo direto: todo documento conectado deve mencionar miocardite/
    miopericardite explicitamente no próprio texto — evita link por
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert "miocardi" in texto or "miopericardi" in texto, (
            f"{slug}: documento vinculado não menciona miocardite/miopericardite no texto"
        )
