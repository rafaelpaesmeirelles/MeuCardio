...
já existente cuidados-paliativos-cardiovasculares (área cardiogeriatria)
...
Diferente de uma doença, esta ficha é um FRAMEWORK de cuidado paliativo
aplicado EM PARALELO ao tratamento cardiovascular ativo (não é
sinônimo de fim de vida nem de suspensão de tratamento) — tinha apenas
metadados de catalogação e 2 related_document_slugs.

O lote não cria nenhum documento, checklist, trilha ou material novo —
conecta a ficha a 7 itens já publicados e revisados em content/ (5
novos + 2 já existentes), todos fora de Farmacologia/Calculadoras/
Exames.
...
SLUG = "cuidados-paliativos-cardiovasculares"
...
# Já existiam nos 2 related_document_slugs originais, antes deste lote —
# ambos também em insuficiencia-cardiaca-no-idoso (paliação terminal em IC).
DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    "marca-passo-e-cdi-no-muito-idoso-indicacao-e-desativacao-em-fim-de-vida",
    "cuidados-paliativos-e-transicao-de-cuidado-na-insuficiencia-cardiaca-terminal-do-muito-idoso",
}
...
def test_catalogacao_original_preservada():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Cuidados paliativos em doença cardiovascular avançada"
    assert "paliação cardiológica" in (item.get("aliases") or [])
    assert item.get("category") == "cuidado_centrado_pessoa"
    assert item.get("subtype") == "paliativo"
...
def test_related_document_slugs_sao_todos_sobre_cuidados_paliativos():
    item = _load_doencas()[SLUG]
    documentos = _all_document_paths()
    termos = ("paliativ", "hospice", "fim de vida", "metas de cuidado")
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(t in texto for t in termos), (
            f"{slug}: documento vinculado não menciona cuidados paliativos/fim de vida no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "conversar-sobre-o-futuro-do-tratamento-planejamento-antecipado-de-cuidados"
    materiais = {
        x["slug"]
