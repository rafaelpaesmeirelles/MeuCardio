...
existente atresia-tricuspide (área cardiopediatria) em doencas/metadados.json.

Ficha estava completeness=basico, 1 related_document_slug pré-existente,
patient_material_slug já preenchido. Não estava sendo trabalhada por nenhuma
PR aberta no momento da produção (verificado via `gh pr list --search`).
...
TERMOS_TEMA = ("fontan", "atresia tricúspide", "ventrículo único", "ventriculo unico", "glenn", "shunt sistêmico-pulmonar", "blalock")

DOCUMENTOS_COMPARTILHADOS_COM_OUTRAS_FICHAS = {
    # também em transposicao-das-grandes-arterias
    ...
    # também em fisiologia-ventriculo-unico
    "tromboprofilaxia-na-circulacao-de-fontan",
    "descompensacao-aguda-da-circulacao-de-fontan",
    # também em dor-toracica-pediatrica
    # (descompensacao-aguda-da-circulacao-de-fontan já listado acima)
    # também em cardiopatia-congenita-gravidez
    # (circulacao-de-fontan-e-gestacao-desfechos-maternos-e-fetais já listado acima)
    # também em cardiopatia-congenita-do-adulto
    # (tromboprofilaxia-na-circulacao-de-fontan e circulacao-de-fontan-e-transposicao-das-grandes-arterias-no-adulto já listados acima)
}
...
def test_texto_com_acentuacao_correta_do_portugues():
    ...
    for palavra in ("não ", "cardíaca", "circulação", "cianose"):
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona Fontan/atresia tricúspide/ventrículo único no texto"
        )
...
def test_patient_material_slug_resolve():
    ...
    assert material == "atresia-tricuspide"
    materiais = {
        x["slug"]
