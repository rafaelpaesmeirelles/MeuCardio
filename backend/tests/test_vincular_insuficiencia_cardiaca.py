...
completa `insuficiencia-cardiaca` (área geral, ficha flagship) em
...
Critério aplicado com rigor extra por ser tema muito central no corpus:
só aceitos documentos-hub gerais/transversais da síndrome, rejeitados
documentos de subtipo/fenótipo específico (ICFEr, ICFEp) já cobertos por
documentos próprios, e comorbidades associadas (síndrome cardiorrenal).
"""
...
TERMOS_TEMA = ("insuficiência cardíaca", "insuficiencia cardiaca")
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Insuficiência cardíaca"
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona insuficiência cardíaca no texto"
        )
