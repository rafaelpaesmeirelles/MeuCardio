...
completa `sindrome-coronariana-cronica` (área geral) em
...
SLUG = "sindrome-coronariana-cronica"

PASTAS_NAO_DOCUMENTO = ("Farmacologia", "Calculadoras", "Exames")
TERMOS_TEMA = ("angina", "coronarian", "isquem", "revasculariz")
...
def test_catalogacao_e_conteudo_clinico_preexistente_preservados():
    item = _load_doencas()[SLUG]
    assert item.get("name") == "Síndrome coronariana crônica"
    assert item.get("area") == "geral"
    assert item.get("category") == "doenca_coronariana"
    assert item.get("prevalence_rank") == 4
...
def test_related_document_slugs_mencionam_tema():
    ...
    for slug in item.get("related_document_slugs") or []:
        ...
        assert any(termo in texto for termo in TERMOS_TEMA), (
            f"{slug}: documento vinculado não menciona angina/coronariana/isquemia/revascularização no texto"
        )
