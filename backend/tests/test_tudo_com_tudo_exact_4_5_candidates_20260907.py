...
FIXTURE = ROOT / "scripts/fixtures/tudo_com_tudo_exact_4_5_candidates_20260907.json"
...
def test_fixture_exact_4_5_tem_45_relacoes_em_10_doencas():
    rows = _fixture()
    assert len(rows) == 45
    assert len({row["source_disease_slug"] for row in rows}) == 10


def test_fixture_exact_4_5_esta_persistida_e_revisada():
...
def test_fixture_exact_4_5_respeita_politica_clinica():
    for item in _fixture():
        validar_relacao_clinica(
            ...
            evidence_source="scripts/fixtures/tudo_com_tudo_exact_4_5_candidates_20260907.json",
            extra={"review_note": "coorte exact 4-5 revisada"},
        )


def test_manifesto_exact_4_5_sem_duplicatas():
    relations = _relations()
    keys = [
        (
