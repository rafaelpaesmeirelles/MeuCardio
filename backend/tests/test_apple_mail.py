"""app/services/apple_mail.py — IMAP/SMTP com senha específica de app, sem
rede real: mesmo padrão de dublê de test_yahoo_mail.py, adaptado pros nomes
de pasta do iCloud ("Sent Messages", "Junk", não "Sent"/"Bulk" da Yahoo).
"""
...
class _IMAPFalso:
    """Simula só os métodos que apple_mail.py chama: login, list, select,
...
def _credenciais() -> dict:
    return {"username": "titular@icloud.com", "app_specific_password": "senha-de-app-123"}


def test_diagnose_login_ok(monkeypatch):
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: _IMAPFalso({"INBOX": {}}))
    resultado = apple_mail.diagnose(_credenciais())
    assert resultado == {"ok": True, "provider": "apple_mail"}
...
def test_conectar_imap_login_recusado_vira_erro_tratado(monkeypatch):
    ...
    monkeypatch.setattr(apple_mail.imaplib, "IMAP4_SSL", _imap_ssl_falso)
    with pytest.raises(AppleMailError) as excinfo:
        apple_mail._conectar_imap(_credenciais())
    assert excinfo.value.status_code == 401


def test_credenciais_ausentes_nao_tenta_conectar():
    with pytest.raises(AppleMailError):
        apple_mail._conectar_imap({})


def test_list_folders_filtra_so_pastas_conhecidas(monkeypatch):
    fake = _IMAPFalso({"INBOX": {}, "Sent Messages": {}, "Notas": {}})
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: fake)
    pastas = apple_mail.list_folders(_credenciais())
    nomes = {p["folderId"] for p in pastas}
    assert nomes == {"INBOX", "Sent Messages"}
...
def test_list_messages_mais_recente_primeiro_e_status_lida(monkeypatch):
    ...
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: fake)
    resultado = apple_mail.list_messages(_credenciais(), folder=None, limit=10, start=1)
    ...
    assert resultado[0]["provider"] == "apple"
...
def test_list_messages_pasta_inexistente_devolve_404(monkeypatch):
    ...
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: fake)
    with pytest.raises(AppleMailError) as excinfo:
        apple_mail.list_messages(_credenciais(), folder="NaoExiste", limit=10, start=1)
...
def test_get_message_inexistente_devolve_404(monkeypatch):
    ...
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: fake)
    with pytest.raises(AppleMailError) as excinfo:
        apple_mail.get_message(_credenciais(), "INBOX:999")
    assert excinfo.value.status_code == 404
...
def test_act_on_messages_marcar_lida_e_nao_lida(monkeypatch):
    ...
    apple_mail.act_on_messages(_credenciais(), ["INBOX:7"], "lida")
    assert "\\Seen" in mensagens["INBOX"][b"7"]["flags"]

    apple_mail.act_on_messages(_credenciais(), ["INBOX:7"], "nao_lida")
...
def test_act_on_messages_mover_nao_suportado(monkeypatch):
    ...
    monkeypatch.setattr(apple_mail, "_conectar_imap", lambda credentials: fake)
    with pytest.raises(AppleMailError):
        apple_mail.act_on_messages(_credenciais(), ["INBOX:1"], "mover")
...
def test_send_message_envia_multipart_html(monkeypatch):
    enviados: list = []
    monkeypatch.setattr(apple_mail.smtplib, "SMTP", lambda host, port, timeout=None: _SMTPFalso(enviados))
    resultado = apple_mail.send_message(
...
def test_send_message_sem_destinatario_valido_e_422():
    with pytest.raises(AppleMailError) as excinfo:
        apple_mail.send_message(_credenciais(), to="", subject="Oi", html="<p>x</p>")
    assert excinfo.value.status_code == 422


def test_send_message_login_recusado_vira_401(monkeypatch):
    monkeypatch.setattr(
        apple_mail.smtplib, "SMTP",
        lambda host, port, timeout=None: _SMTPFalso([], falhar_login=True),
    )
    with pytest.raises(AppleMailError) as excinfo:
        apple_mail.send_message(_credenciais(), to="destino@example.com", subject="Oi", html="<p>x</p>")
    assert excinfo.value.status_code == 401
