...
def oauth_scopes(provider: str, *, calendar_write: bool, contacts: bool) -> list[str]:
    if provider == "google_calendar":
        ...
        scopes.append(
            ...
            if calendar_write else "https://www.googleapis.com/auth/calendar.events.readonly"
        )
        if contacts:
    ...
    *,
    ...
    if not oauth_configured(provider):
        raise ConnectorError(
            ...
            "O administrador ainda precisa cadastrar o cliente OAuth deste provedor.",
            status_code=503,
        )
...
def _consume_state(db: Session, provider: str, state: str) -> tuple[IntegrationOAuthState, str]:
    ...
    if not record or record.used_at or not expires_at or expires_at <= now:
        raise ConnectorError("invalid_oauth_state", "Autorização expirada ou já utilizada.", status_code=400)
    verifier = decifrar_campo(record.code_verifier_cipher, record.owner_id)
    record.used_at = now
    db.commit()
...
def _token_payload(provider: str, code: str, verifier: str) -> tuple[str, dict[str, str]]:
    ...
    return f"{_microsoft_authority()}/token", {
        **common,
...
def _normalize_token(body: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    if not body.get("access_token"):
        raise ConnectorError("oauth_token_error", "O provedor não devolveu token de acesso.", status_code=502)
    expires_in = max(60, int(body.get("expires_in") or 3600))
    result = {
        "access_token": str(body["access_token"]),
        "refresh_token": str(body.get("refresh_token") or (previous or {}).get("refresh_token") or ""),
        "expires_at": (datetime.now(timezone.utc) + timedelta(seconds=expires_in)).isoformat(),
        "token_type": str(body.get("token_type") or "Bearer"),
        "scope": str(body.get("scope") or (previous or {}).get("scope") or ""),
    }
    if not result["refresh_token"]:
        raise ConnectorError(
            "oauth_refresh_token_missing",
            "O provedor não concedeu acesso offline. Remova a autorização anterior e conecte novamente.",
            status_code=409,
        )
    return result
...
def _provider_profile(provider: str, token: str) -> dict[str, str]:
    ...
    if provider == "google_calendar":
        ...
        if response.status_code >= 400:
            raise ConnectorError("oauth_profile_error", f"Google profile HTTP {response.status_code}.")
        body = response.json()
        return {
            "id": str(body.get("sub") or body.get("email") or ""),
            "name": str(body.get("name") or body.get("email") or "Conta Google"),
            "email": str(body.get("email") or ""),
        }
    response = httpx.get(
        f"{MICROSOFT_GRAPH}/me?$select=id,displayName,mail,userPrincipalName",
        headers=headers,
        timeout=20.0,
    )
    if response.status_code >= 400:
        raise ConnectorError("oauth_profile_error", f"Microsoft Graph HTTP {response.status_code}.")
    body = response.json()
    email = body.get("mail") or body.get("userPrincipalName") or ""
    return {"id": str(body.get("id") or email), "name": str(body.get("displayName") or email or "Conta Microsoft"), "email": str(email)}


def complete_oauth(db: Session, *, provider: str, code: str, state: str) -> CalendarIntegration:
    record, verifier = _consume_state(db, provider, state)
    token_url, payload = _token_payload(provider, code, verifier)
    try:
        response = httpx.post(token_url, data=payload, timeout=30.0)
    except httpx.HTTPError as exc:
        raise ConnectorError("oauth_network_error", "Falha de rede ao concluir a autorização.", retryable=True) from exc
    if response.status_code >= 400:
        raise ConnectorError("oauth_exchange_error", f"O provedor recusou a autorização (HTTP {response.status_code}).", status_code=400)
    token = _normalize_token(response.json())
    profile = _provider_profile(provider, token["access_token"])
    integration = db.query(CalendarIntegration).filter(
        CalendarIntegration.owner_id == record.owner_id,
        CalendarIntegration.provider == provider,
        CalendarIntegration.external_account_id == profile["id"],
    ).first()
    capabilities = {
        "read_appointments": True,
        "incremental_sync": True,
        "create_appointment": bool(record.request_data.get("calendar_write")),
    ...
    if not integration:
        integration = CalendarIntegration(
            ...
            provider=provider,
            display_name=profile["email"] or profile["name"],
            external_account_id=profile["id"],
            sync_strategy="bidirectional" if record.request_data.get("calendar_write") else "external_authoritative",
            configuration={"calendar_id": "primary"} if provider == "google_calendar" else {},
        )
        db.add(integration)
        db.flush()
    ...
    store_integration_credentials(integration, token)
    db.commit()
    db.refresh(integration)
    return integration


def ensure_fresh_credentials(db: Session, integration: CalendarIntegration) -> dict[str, Any]:
    ...
    if not refresh_token:
        raise ConnectorError("reauth_required", "A conta precisa ser conectada novamente.", status_code=409)
    ...
    try:
        ...
        raise ConnectorError("oauth_refresh_network_error", "Falha de rede ao renovar a autorização.", retryable=True) from exc
    if response.status_code >= 400:
        integration.status = "reauth_required"
        integration.last_error_code = "reauth_required"
        integration.last_error_message = "A autorização externa expirou ou foi revogada."
        db.commit()
...
def disconnect_integration(db: Session, integration: CalendarIntegration) -> None:
    ...
    if integration.provider == "google_calendar" and credentials.get("access_token"):
        try:
            ...
            pass
    integration.credentials_cipher = None
    integration.cursor_cipher = None
    integration.contacts_cursor_cipher = None
    integration.enabled = False
    integration.write_enabled = False
    integration.contacts_enabled = False
