"""Escritor de PDF em Python puro — só stdlib (zlib, struct, unicodedata).

Existe porque não há nenhuma ferramenta de PDF instalada e não é possível
instalar: reportlab, weasyprint, fpdf, pandoc, wkhtmltopdf, chromium e
libreoffice estão todos ausentes, e não há `pip`. Em vez de acrescentar uma
dependência pesada ao container para gerar dois tipos de documento, o formato
é escrito diretamente.

Escopo deliberadamente pequeno — é o necessário para documento de texto com
...
- imagem PNG com canal alfa, decodificada aqui mesmo (o alfa vira /SMask, para
  a logo funcionar tanto sobre branco quanto sobre fundo escuro);
...
class PDF:
    ...
    def poligono(self, pontos: list[tuple[float, float]],
                 preenchimento: tuple[float, float, float] | None = None,
                 traco: tuple[float, float, float] | None = None,
                 espessura: float = 1.0) -> None:
        """Polígono fechado — existe para o losango de decisão do fluxograma,
        que é a única forma da árvore que um retângulo não representa."""
        if len(pontos) < 3:
            return
        partes = [f"{pontos[0][0]:.2f} {pontos[0][1]:.2f} m"]
        partes += [f"{x:.2f} {y:.2f} l" for x, y in pontos[1:]]
        partes.append("h")
        if preenchimento:
            r, g, b = preenchimento
            partes.insert(0, f"{r:.4f} {g:.4f} {b:.4f} rg")
        if traco:
            r, g, b = traco
            partes.insert(0, f"{r:.4f} {g:.4f} {b:.4f} RG {espessura:.2f} w")
        partes.append("B" if (preenchimento and traco) else ("f" if preenchimento else "S"))
        self._op(" ".join(partes).encode())

    def retangulo_traco(self, x: float, y: float, larg: float, alt: float,
                        preenchimento: tuple[float, float, float],
                        traco: tuple[float, float, float],
                        espessura: float = 1.0) -> None:
        """Retângulo preenchido com contorno, numa só passada."""
        r, g, b = preenchimento
        tr, tg, tb = traco
        self._op(
            f"{r:.4f} {g:.4f} {b:.4f} rg {tr:.4f} {tg:.4f} {tb:.4f} RG "
            f"{espessura:.2f} w {x:.2f} {y:.2f} {larg:.2f} {alt:.2f} re B".encode()
        )
