...
def _gmail_body(payload: dict[str, Any]) -> tuple[str, str]:
    html = ""; text = ""; stack = [payload]
    while stack:
        part = stack.pop(); mime = str(part.get("mimeType") or ""); data = str((part.get("body") or {}).get("data") or "")
        if data and mime == "text/html" and not html: html = _b64url_decode(data)
        elif data and mime == "text/plain" and not text: text = _b64url_decode(data)
        stack.extend(reversed(part.get("parts") or []))
    return html, text


def _gmail_message(raw: dict[str, Any], *, full: bool = False) -> dict[str, Any]:
    payload = raw.get("payload") or {}; headers = _gmail_headers(payload); labels = set(raw.get("labelIds") or [])
    result: dict[str, Any] = {
        "messageId": str(raw.get("id") or ""), "id": str(raw.get("id") or ""),
        "subject": headers.get("subject") or "(sem assunto)", "fromAddress": headers.get("from") or "",
        "toAddress": headers.get("to") or "", "ccAddress": headers.get("cc") or "",
        "receivedTime": str(raw.get("internalDate") or ""), "summary": str(raw.get("snippet") or ""),
    ...
    if full:
        html, text = _gmail_body(payload); result["htmlContent"] = html or None; result["content"] = html or escape(text).replace("\n", "<br>")
    return result
...
def _graph_message(raw: dict[str, Any], *, full: bool = False) -> dict[str, Any]:
    result: dict[str, Any] = {
        "messageId": str(raw.get("id") or ""), "id": str(raw.get("id") or ""),
        "subject": str(raw.get("subject") or "(sem assunto)"), "fromAddress": _address(raw.get("from") or raw.get("sender")),
        "toAddress": _addresses(raw.get("toRecipients")), "ccAddress": _addresses(raw.get("ccRecipients")),
        "receivedTime": str(raw.get("receivedDateTime") or raw.get("sentDateTime") or ""), "summary": str(raw.get("bodyPreview") or ""),
        "status": "1" if raw.get("isRead") else "0", "hasAttachment": int(bool(raw.get("hasAttachments"))), "provider": "microsoft",
    }
    if full:
        body = raw.get("body") or {}; content = str(body.get("content") or "")
        result["htmlContent"] = content if str(body.get("contentType", "")).lower() == "html" else None; result["content"] = content
    return result


def list_folders(db: Session, integration: CalendarIntegration) -> list[dict[str, Any]]:
    if integration.provider == "google_calendar":
        body = _request(db, integration, "GET", f"{_GMAIL}/labels").json(); visible = {"INBOX", "DRAFT", "SENT", "SPAM", "TRASH", "IMPORTANT", "STARRED"}
        return [{"folderId": item["id"], "id": item["id"], "folderName": item.get("name", item["id"]), "folderType": item["id"]} for item in body.get("labels", []) if item.get("id") in visible or item.get("type") == "user"]
    body = _request(db, integration, "GET", f"{_GRAPH}/mailFolders", params={"$top": "100", "$select": "id,displayName,wellKnownName"}).json()
    return [{"folderId": item["id"], "id": item["id"], "folderName": item.get("displayName", "Pasta"), "folderType": item.get("wellKnownName") or item.get("displayName")} for item in body.get("value", [])]
...
def list_messages(db: Session, integration: CalendarIntegration, *, folder: str | None, limit: int, start: int) -> list[dict[str, Any]]:
    ...
    if integration.provider == "google_calendar":
        ...
        return result
    folder_id = quote(folder or "inbox", safe=""); fields = "id,subject,from,sender,toRecipients,ccRecipients,receivedDateTime,sentDateTime,bodyPreview,isRead,hasAttachments"
    body = _request(db, integration, "GET", f"{_GRAPH}/mailFolders/{folder_id}/messages", params={"$top": str(limit), "$skip": str(start - 1), "$orderby": "receivedDateTime desc", "$select": fields}).json()
...
def get_message(db: Session, integration: CalendarIntegration, message_id: str) -> dict[str, Any]:
    ...
    if not raw.get("isRead"): _request(db, integration, "PATCH", f"{_GRAPH}/messages/{encoded}", json={"isRead": True})
    return _graph_message(raw, full=True)


def act_on_messages(db: Session, integration: CalendarIntegration, message_ids: list[str], action: str, *, destination: str | None = None) -> None:
    for message_id in message_ids:
        ...
        if integration.provider == "google_calendar":
            ...
            elif action == "mover" and destination: add, remove = [destination], ["INBOX"]
            else: continue
            _request(db, integration, "POST", f"{_GMAIL}/messages/{encoded}/modify", json={"addLabelIds": add, "removeLabelIds": remove})
        elif action in {"lida", "nao_lida"}: _request(db, integration, "PATCH", f"{_GRAPH}/messages/{encoded}", json={"isRead": action == "lida"})
...
def delete_message(db: Session, integration: CalendarIntegration, message_id: str) -> None:
    ...
    if integration.provider in {"yahoo_mail", "apple_icloud"}:
        ...
        move_to_trash(credentials, message_id, host="imap.mail.yahoo.com" if integration.provider == "yahoo_mail" else "imap.mail.me.com", trash_folder="Trash", error_cls=ExternalMailError)
        return
    encoded = quote(message_id, safe="")
    if integration.provider == "google_calendar": _request(db, integration, "POST", f"{_GMAIL}/messages/{encoded}/trash")
    elif integration.provider == "microsoft_365": _request(db, integration, "DELETE", f"{_GRAPH}/messages/{encoded}")
    else: raise ExternalMailError("Provedor externo não suportado para exclusão.", status_code=422)
