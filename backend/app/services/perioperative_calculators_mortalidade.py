"""S-MPM para mortalidade por todas as causas em 30 dias.
...
def _smpm(d: dict) -> dict:
    asa = int(d["asa"]); proc = d["risco_procedimento"]
...
def _smpm_txt(r: dict) -> str:
    return (f"S-MPM {r['score']}/9 — Classe {r['classe']}; mortalidade estimada em 30 dias {r['mortalidade_30d']} na classificação original. "
            "É mortalidade cirúrgica global, não risco cardíaco específico. A classe do procedimento deve seguir a metodologia do S-MPM; se não puder ser classificada com segurança, usar VERIFICAÇÃO HUMANA NECESSÁRIA.")
...
MORTALITY_PERIOPERATIVE_REGISTRY: dict[str, Calculator] = {
    "s-mpm": Calculator(
        ...
        fields=[
            ...
            Field("risco_procedimento", "Classe de risco do procedimento no S-MPM", "select", options=[{"value": "baixo", "label": "Baixo — 0"}, {"value": "intermediario", "label": "Intermediário — 1"}, {"value": "alto", "label": "Alto — 2"}], help="A taxonomia do S-MPM não é idêntica às categorias ESC/AHA."),
        ...
        limitations=["Endpoint é mortalidade global em 30 dias, não MACE/MICA.", "A classificação do procedimento foi empiricamente derivada e pode divergir de ESC/AHA.", "Não usar classe aproximada sem registrar VERIFICAÇÃO HUMANA NECESSÁRIA."],
    )
}