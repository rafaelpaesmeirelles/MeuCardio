...
export default function AtelierHomeView(props: Props) {
  ...
  return <>
    <header className="atelier-header">
      ...
      <details ref={accountRef} className="atelier-account"><summary aria-label={`Menu da conta de ${usuario?.full_name || "usuário"}`}>{props.identity}</summary><div><CorviaThemeSelector variant="menu" /><Link to="/minha-conta">Minha conta</Link><Link to="/corvia-mail">CorVIA Mail</Link><Link to="/favoritos">Notas & Favoritos</Link>{usuario?.role === "admin" && <Link to="/admin">Administração</Link>}<Link to="/tour">Conhecer a plataforma</Link><button type="button" onClick={() => void sair()}>Sair</button></div></details>
    </header>
