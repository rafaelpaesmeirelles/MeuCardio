...
type Encounter = {
  id:number; encounter_type:string; status:string; started_at:string; finalized_at:string|null;
...
export default function Prontuario(){
  ...
  const [pacientes,setPacientes]=useState<Paciente[]>([]),[encounters,setEncounters]=useState<Encounter[]>([]);
  ...
  async function finalizar(){
    ...
    try{const e=await api.post<Encounter>(`/pacientes/${pid}/atendimentos/${salvo.id}/finalizar`);setEncounters(x=>[e,...x.filter(i=>i.id!==e.id)]);setEditor(false);setEditando(null);setForm(VAZIO);}catch(e){setErro(e instanceof Error?e.message:"Falha ao finalizar atendimento.");}finally{setSalvando(false);}
  }
