...
type HeartMetrics = {
  ...
  wallet_accounting?: { currency: "BRL"; actual_cost_micros: number; reserved_cost_micros: number; unknown_operations: number; scope: string };
};
...
export default function AdminAIOperations() {
  ...
  return <div className="pagina cai-page cai-admin-ai">
    ...
    <section className="cai-admin-summary">
      ...
      <article><span><Icone nome="calculadora" /></span><small>Custo registrado · Heart Team</small><strong>{heart?.wallet_accounting ? money(heart.wallet_accounting.actual_cost_micros) : "Indisponível"}</strong><em>{heart?.wallet_accounting ? `${money(heart.wallet_accounting.reserved_cost_micros)} reservado · ${heart.wallet_accounting.unknown_operations} em reconciliação` : "Aguardando dados da carteira"}</em></article>
    </section>
    <p style={{ fontSize: "0.85rem", color: "var(--texto-secundario)" }}>O custo do Heart Team corresponde aos registros desde a ativação da carteira. Estimativas históricas e dados do WhatsApp são apresentados separadamente.</p>
