...
const BENEFICIOS = [
  {
    ...
    detail: "O token de recuperação é temporário, vinculado à conta e de uso único.",
    tone: "cyan" as const,
  },
  {
    icon: "documento" as const,
    title: "Proteção de dados",
    detail: "A senha não é enviada por e-mail e fica armazenada somente de forma protegida.",
    tone: "violet" as const,
  },
  {
    icon: "sincronizar" as const,
    title: "Ambiente confiável",
    detail: "Depois da troca, o acesso volta ao fluxo normal do Clinical OS.",
    tone: "green" as const,
  },
];
...
export default function RedefinirSenha() {
  ...
  const conteudo = !token ? (
    <div className="prehome-confirmation">
      <div className="prehome-confirmation__icon prehome-confirmation__icon--pending"><Icone nome="emergencia" /></div>
      <h2>Link inválido</h2>
      <p>O token de redefinição não está presente. Solicite um novo link pelo canal seguro.</p>
      <Link to="/esqueci-senha" className="prehome-primary"><span>Solicitar novo link</span><Icone nome="seta" /></Link>
    </div>
  ) : feito ? (
    <div className="prehome-confirmation" role="status">
      ...
      <p>Pronto. Você será levado para {alvo === "email" ? "o CorVIA Mail" : "o login do Clinical OS"}.</p>
    </div>
  ) : (
    <form className="login-formulario" onSubmit={(e) => { e.preventDefault(); void enviar(); }}>
      <div className="login-campo">
        ...
        <div className="prehome-password-meter" data-level={nivelForca} aria-label={`Força da senha: ${rotuloForca}`}>
          <i /><i /><i /><i /><span>{senha ? rotuloForca : ""}</span>
        </div>
      </div>
      ...
      <div className="login-campo">
        ...
        {confirmacao.length > 0 && !senhasBatem && (
          <p className="login-formulario__secao-nota" style={{ color: "#ff9bad" }}>As senhas não coincidem.</p>
        )}
      </div>

      <div>
        <p className="prehome-card__eyebrow" style={{ marginBottom: 9 }}>Sua senha</p>
        <ul className="prehome-password-rules">
          <li data-ok={regras.tamanho}><Icone nome="check" /> Mínimo de 8 caracteres</li>
          <li data-ok={regras.maiuscula}><Icone nome="check" /> Letra maiúscula — recomendado</li>
          <li data-ok={regras.minuscula}><Icone nome="check" /> Letra minúscula — recomendado</li>
          <li data-ok={regras.numero}><Icone nome="check" /> Número — recomendado</li>
          <li data-ok={regras.especial}><Icone nome="check" /> Caractere especial — recomendado</li>
        </ul>
      </div>
  ...
  return (
    <main className="login prehome prehome--recovery">
      <PreHomeBrand
        title={<>Redefina sua senha com <strong>segurança</strong></>}
        description={<>Crie uma nova senha exclusiva para retomar o acesso ao seu Clinical Command Center.</>}
        benefits={BENEFICIOS}
      />
      ...
      <section className="prehome-access" aria-labelledby="reset-titulo">
        <div className="prehome-card">
          {token && !feito && (
            <div className="prehome-status-chip"><Icone nome="check" /> Link de recuperação recebido</div>
          )}
          <header className="prehome-card__header" style={{ textAlign: "center" }}>
            <h2 id="reset-titulo">{alvo === "email" ? "Definir nova senha do CorVIA Mail" : "Definir nova senha"}</h2>
            <p style={{ marginLeft: "auto", marginRight: "auto" }}>
              Crie uma senha forte e exclusiva para acessar o Clinical OS.
            </p>
          </header>
