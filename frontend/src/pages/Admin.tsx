      ...
      {aberto && (
        <div className="cartao" style={{ marginTop: 8, padding: "0.75rem" }}>
          ...
          {carregando ? <Carregando texto="Consultando…" /> : (
            <>
              <input type="email" aria-label="Segundo e-mail independente para recuperação" value={email} placeholder="email.externo@provedor.com"
...
function SolicitacaoCard({ u, aoDecidir }: { u: Usuario; aoDecidir: () => void }) {
  ...
  return <div className="cartao cartao--clinico">
    ...
    <div style={{ fontSize: "0.88rem", marginTop: 4 }}>{[u.profession, [u.council_name, u.council_number, u.council_state].filter(Boolean).join(" ")].filter(Boolean).join(" — ")}{u.specialty && <> · {u.specialty}</>}</div>
    ...
    {u.role !== "admin" && <RecoveryEditor u={u} />}
    <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: "0.8rem", flexWrap: "wrap" }}>
      <select aria-label="Perfil de acesso solicitado" value={perfil} onChange={(e) => setPerfil(e.target.value)} style={{ maxWidth: 180 }}>{PERFIS.map((p) => <option key={p.valor} value={p.valor}>{p.rotulo}</option>)}</select>
    ...
    {rejeitando && <div style={{ marginTop: "0.6rem" }}>
      <label htmlFor={`admin-rejection-reason-${u.id}`}>Motivo da rejeição (opcional)</label>
      <input id={`admin-rejection-reason-${u.id}`} placeholder="Motivo (opcional, fica registrado)" value={nota} onChange={(e) => setNota(e.target.value)} />
      <button className="botao botao--secundario" style={{ marginTop: 6 }} onClick={() => decidir(false)} disabled={enviando}>Confirmar rejeição</button>
    </div>}
...
function EmailTransacionalPanel() {
  ...
  return <div className="cartao cartao--clinico" style={{ marginTop: "0.8rem", maxWidth: 760 }}>
    ...
    <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}><input type="email" aria-label="E-mail externo para teste de entrega" placeholder="e-mail externo para teste" value={destino} onChange={(e) => setDestino(e.target.value)} style={{ flex: "1 1 260px" }} /><button className="botao" disabled={!emailValido(destino) || testando || !verde} onClick={testar}>{testando ? "Testando…" : "Enviar teste real"}</button></div>
...
export default function Admin() {
  ...
  return <>
    ...
    <Link to="/admin/mudancas-clinicas" className="cartao cartao--clinico" style={{ display: "block", marginTop: "0.8rem", maxWidth: 760, textDecoration: "none" }}><strong>Mudanças de Conduta Baseadas em Novas Evidências para Aprovação →</strong><p style={{ margin: "0.3rem 0 0", color: "var(--texto-secundario)", fontSize: "0.86rem" }}>Compare as sugestões de conduta e tratamento, consulte as fontes e decida sobre sua aplicação no conteúdo do CorVIA.</p></Link>
    ...
    {lista === null ? <Carregando /> : <>
      ...
      <div className="cartao cartao--clinico" style={{ marginTop: "0.8rem", maxWidth: 760 }}><p className="eyebrow">Nova conta</p>
        ...
        {!investidorNovo && <><h3 style={{ fontSize: "1rem", marginTop: "1rem" }}>Local de trabalho (opcional)</h3><div className="grade grade--2"><input aria-label="Instituição, clínica ou consultório" placeholder="Instituição, clínica ou consultório" value={novo.workplace_name} onChange={(e) => setNovo({ ...novo, workplace_name: e.target.value })} /><input aria-label="Setor/unidade" placeholder="Setor/unidade" value={novo.workplace_department} onChange={(e) => setNovo({ ...novo, workplace_department: e.target.value })} /><input aria-label="Cargo/função" placeholder="Cargo/função" value={novo.workplace_role} onChange={(e) => setNovo({ ...novo, workplace_role: e.target.value })} /><input aria-label="Outras informações" placeholder="Outras informações" value={novo.workplace_notes} onChange={(e) => setNovo({ ...novo, workplace_notes: e.target.value })} /></div><label style={{ display: "flex", gap: 8, alignItems: "center", fontWeight: 400, marginTop: 8 }}><input type="checkbox" style={{ width: "auto" }} checked={novo.include_workplace_on_documents} onChange={(e) => setNovo({ ...novo, include_workplace_on_documents: e.target.checked })} />Incluir local de trabalho nos documentos</label><label style={{ display: "flex", gap: 8, alignItems: "center", fontWeight: 400, marginTop: 8 }}><input type="checkbox" style={{ width: "auto" }} checked={novo.profile_completion_required} onChange={(e) => setNovo({ ...novo, profile_completion_required: e.target.checked })} />No primeiro acesso, completar dados pessoais e profissionais</label></>}
