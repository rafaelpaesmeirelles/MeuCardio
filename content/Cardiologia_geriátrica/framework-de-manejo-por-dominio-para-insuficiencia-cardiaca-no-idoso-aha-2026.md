---
title: "Framework de Manejo por Domínio para Insuficiência Cardíaca no Idoso: Scientific Statement da AHA 2026"
slug: framework-de-manejo-por-dominio-para-insuficiencia-cardiaca-no-idoso-aha-2026
theme: "Cardiologia geriátrica"
kind: documento
review_status: revisado
source_refs: ["Strategies for Optimizing Heart Failure Care in the Older Adult: A Scientific Statement From the American Heart Association. Circulation. Publicado online 18/06/2026. DOI: 10.1161/CIR.0000000000001437. PMID: 42312386."]
review_note: "Classificação corrigida em 26/08/2026: a fonte primária se identifica como AHA Scientific Statement e propõe um framework de cuidado, sem classes/forças de recomendação de diretriz. Mantido como documento de consenso e conectado aos módulos clínicos locais."
legacy_source: "Documento novo, escrito em 19/08/2026. Esta pasta cobre extensamente cenários clínicos específicos de IC no idoso (choque cardiogênico, anemia, apneia do sono, etc.), mas não tinha nenhum documento sobre framework GERAL e estruturado de otimização de cuidado de IC no muito idoso — peça que organiza o manejo em domínios, além das terapias farmacológicas individuais."
---

# Framework de Manejo por Domínio para Insuficiência Cardíaca no Idoso: Scientific Statement da AHA 2026

## Por que um framework de domínios, e não só mais uma diretriz de fármaco
Os documentos já existentes nesta pasta cobrem cenários clínicos específicos de IC no idoso — mas a barreira real para entregar terapia medicamentosa otimizada guiada por diretriz (TMOG) ao paciente muito idoso raramente é só farmacológica. Este scientific statement propõe um **framework de manejo por domínio**, organizando as estratégias de otimização além da prescrição isolada.

## O documento
*Circulation*, publicado online 18/06/2026, DOI 10.1161/CIR.0000000000001437, PMID 42312386. É Scientific Statement — síntese/consenso de especialistas, não ensaio clínico primário.

## O "Domain Management Framework"
O documento organiza estratégias de otimização de cuidado de IC no idoso em **quatro domínios**:

1. **Médico**
2. **Cognitivo**
3. **Físico**
4. **Social**

O documento também aborda multimorbidade, polifarmácia, fragilidade, necessidades sociais, preferências do paciente e barreiras de implementação da TMOG.

## Escopo desta síntese
Este texto apresenta a estrutura multidimensional confirmada pela publicação e pelos materiais oficiais da AHA. Decisões farmacológicas ou invasivas específicas devem ser consultadas no documento integral e individualizadas.

## Onde isso se encaixa na prática
Serve como estrutura organizadora para quem já maneja os cenários clínicos específicos cobertos nesta pasta (choque cardiogênico no idoso, anemia na IC do idoso, comprometimento cognitivo como modificador de decisão) — o framework de domínios é o que amarra essas peças individuais numa abordagem sistemática de otimização de cuidado, em vez de decisões isoladas por problema.

## Armadilhas clínicas
- **Tratar este framework como substituto da terapia medicamentosa otimizada guiada por diretriz (TMOG)** — é complemento organizacional à prescrição farmacológica, não alternativa a ela
- **Aplicar os quatro domínios de forma genérica sem individualizar** — o documento propõe estrutura, mas a implementação exige avaliação individual em cada domínio
- **Transformar o framework em prescrição automática** — ele organiza a avaliação; não substitui julgamento clínico, diretrizes terapêuticas nem decisão compartilhada

## Tudo com Tudo

- [Comprometimento cognitivo e demência como modificadores de decisão cardiovascular](comprometimento-cognitivo-e-demencia-como-modificador-de-decisao-cardiovascular.md)
- [Anemia e deficiência de ferro na insuficiência cardíaca do idoso](anemia-e-deficiencia-de-ferro-na-insuficiencia-cardiaca-do-idoso.md)
- [Choque cardiogênico e suporte circulatório mecânico no idoso](choque-cardiogenico-e-suporte-circulatorio-mecanico-no-idoso-idade-cronologica-versus-fragilidade.md)
