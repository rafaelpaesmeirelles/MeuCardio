...
review_note: "Produção científica assistida (Claude, 26/08/2026); pendente de revisão independente e validação médica final. Conferido nesta sessão: seção 7.1 (7.1.1 a 7.1.4) e Tabela 12 da diretriz ESC 2024 de FA lidas na íntegra em reprodução textual do artigo original e na tradução oficial polonesa publicada em Kardiologia Polska; Tabela de Recomendações 14 (classes e níveis) e Figura 7 lidas na tradução oficial polonesa; recomendação de cardioversão elétrica na instabilidade hemodinâmica (classe I, nível C) lida na mesma fonte; frase sobre fármacos por FEVE conferida no resumo da ACC. Derivado dos documentos já publicados no acervo sobre RATE-AF, AFFIRM/EAST-AFNET 4/CASTLE-AF e APAF-CRT. A página do artigo no OUP foi aberta, mas o texto integral não carregou além da seção 3.5. Verificação adversarial (Claude, 26/08/2026): seção 7.1, Tabela 12 e Tabela de Recomendações 14 reconferidas na reprodução textual e no PDF da tradução polonesa (texto extraído com pdftotext), Tabela de Recomendações 15 (cardioversão I C) e Figura 7 conferidas; DOI e PMID batem no Crossref e PubMed; todos os números da tabela de doses conferem. Correções: raiz da árvore passou a excluir explicitamente a FA pré-excitada (WPW), porque a Tabela 12 contraindica todos os fármacos de controle de frequência nesse cenário; removido o número de volume (82) da citação de Kardiologia Polska, não confirmado no PDF; sete links do Tudo com Tudo conferidos como existentes."
source_refs:
