...
- **VERIFICAÇÃO HUMANA NECESSÁRIA**: não foi possível, dentro desta sessão, confirmar contra o texto completo do artigo se há corte de pontuação com recomendação explícita de conduta (por exemplo, um valor a partir do qual o próprio Gage et al. sugerem reavaliação obrigatória) — o que está registrado acima é a tabela de incidência de sangramento por faixa de pontuação, que é o dado central do artigo e foi conferido no resumo indexado no PubMed.
