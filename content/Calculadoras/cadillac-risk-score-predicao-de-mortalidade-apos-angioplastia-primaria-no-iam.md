...
source_refs:
- 'Halkin A et al. Prediction of mortality after primary percutaneous coronary intervention for acute myocardial
  infarction: the CADILLAC risk score. J Am Coll Cardiol. 2005;45:1397-1405. DOI 10.1016/j.jacc.2005.01.041. PMID
  15862409.'
- Estudo independente CADILLAC2021, métodos/resultados. https://pmc.ncbi.nlm.nih.gov/articles/PMC8588705/
- ALPHA PMID28029531.
- ACTION-GWTG PMID29254271.
legacy_source: 'Documento novo, escrito em 09/09/2026. A pasta Calculadoras já cobria escores de risco cirúrgico
  (EuroSCORE II, STS Risk Score), o escore anatômico de complexidade coronariana (SYNTAX e SYNTAX Score II, para
  decisão entre PCI e CABG) e os escores de síndrome coronariana aguda aplicados na apresentação/triagem (TIMI,
  GRACE 2.0, HEART) — mas nenhum documento cobria um escore que prediz mortalidade especificamente DEPOIS de realizada
  a angioplastia primária no IAM, incorporando o resultado angiográfico pós-procedimento (fluxo TIMI) e a função
  ventricular basal. Essa é uma pergunta distinta: não é ''qual a probabilidade de complicação antes de decidir
  a estratégia'' (GRACE/TIMI/HEART) nem ''CABG ou PCI é melhor para esta anatomia'' (SYNTAX), é ''este paciente,
  já submetido à angioplastia primária, tem que prognóstico de curto e longo prazo''. Checagem de colisão feita
  em content/**/*.md, não só nos JSONs: a pasta Doença_coronariana já tem documentos sobre o ENSAIO CADILLAC (Stone
  GW, NEJM 2002, PMID 11919304 — stent vs. PTCA ± abciximabe) e sobre ADMIRAL/EPILOG (outros ensaios de abciximab
  na ICP) — nenhum deles trata do escore de risco derivado posteriormente a partir da coorte CADILLAC, que é o objeto
  deste documento; são complementares, não duplicados, e o ensaio original foi linkado na seção Tudo com Tudo. Verificado
  via PubMed E-utilities nesta sessão (esearch + efetch de texto puro do abstract, PMID confirmado diretamente).'
summary: null
tags: []
evidence_level: null
source_tier: A
...
Sete variáveis do artigo original, com definições operacionais conferidas também nos métodos do estudo independente de 2021 (PMC8588705):
...
| Anemia: hematócrito <39% nos homens ou <36% nas mulheres | 2 |
| Insuficiência renal: clearance de creatinina <60 mL/min | 3 |
| Doença trivascular | 2 |
| Fluxo TIMI pós-procedimento 0–2 | 2 |
...
Os cortes acima são explicitados nos métodos do [estudo independente de 2021](https://pmc.ncbi.nlm.nih.gov/articles/PMC8588705/), que reaplicou o CADILLAC em 341 pacientes com STEMI submetidos a angioplastia primária. O texto usa hematócrito para anemia; não se devem copiar unidades inconsistentes de hemoglobina presentes na tabela dessa publicação.
...
Há avaliações independentes posteriores. O estudo de 2021 (PMC8588705), retrospectivo e unicêntrico, incluiu 341 pacientes e excluiu parada cardíaca ou choque na apresentação. A ausência de mortalidade no grupo de baixo risco até um ano é um resultado dessa amostra, sem significar risco individual zero. A discriminação para eventos adversos intra-hospitalares foi apenas moderada (C=0,66).

Os estudos ALPHA (PMID 28029531) e ACTION-GWTG (PMID 29254271) também avaliaram comparativamente o desempenho do CADILLAC. Portanto, não é correto afirmar ausência de validação posterior com base numa busca restrita por títulos.
...
- Killip II/III e FEVE <40% identificam maior risco prognóstico. O escore não demonstra quem se beneficiará de suporte mecânico nem prediz especificamente choque refratário; tais decisões exigem avaliação hemodinâmica própria.
...
- **Substituir os cortes por intuição clínica** — respeitar as definições da tabela e o contexto pós-angioplastia.
