---
kind: calculadora
legacy_source: 'Documento novo, escrito nesta sessão. Esta pasta já cobre o ensaio
  IABP-SHOCK II (eficácia do balão intra-aórtico) em dois documentos próprios de outras
  pastas (Terapia_intensiva e Doença_coronariana), e cobre o escore SAVE para sobrevida
  pré-ECMO — mas nenhum documento existente descrevia o escore de risco de mortalidade
  derivado da própria população do ensaio/registro IABP-SHOCK II (Pöss J et al., JACC
  2017), pergunta distinta: não é sobre o benefício do balão, é sobre estratificação
  prognóstica no choque cardiogênico pós-IAM já estabelecido, com ou sem balão.'
published: true
review_note: Revisão de fontes concluída em 2026-09-09. Conferida tabela de pesos
  no artigo integral (ilustração central, p.1917). Separadas derivação (n=480; AUROC
  0,74), validação no registro (n=98; 0,79) e CardShock (n=137; 0,73).
review_status: revisado
slug: escore-iabp-shock-ii-estratificacao-de-risco-de-mortalidade-no-choque-cardiogenico-pos-iam
source_refs:
- 'Pöss J, Köster J, Fuernau G, Eitel I, de Waha S, Ouarrak T, Lassus J, Harjola VP,
  Zeymer U, Thiele H, Desch S. Risk Stratification for Patients in Cardiogenic Shock
  After Acute Myocardial Infarction. J Am Coll Cardiol. 2017;69(15):1913-1920. DOI:
  10.1016/j.jacc.2017.02.027. PMID: 28408020'
- 'Harjola VP, Lassus J, Sionis A, Køber L, Tarvasmäki T, Spinar J, Parissis J, Banaszewski
  M, Silva-Cardoso J, Carubelli V, Di Somma S, Tolppanen H, Zeymer U, Thiele H, Nieminen
  MS, Mebazaa A; CardShock Study Investigators, GREAT network. Clinical picture and
  risk prediction of short-term mortality in cardiogenic shock. Eur J Heart Fail.
  2015;17(5):501-509. PMID: 25820680 — coorte de validação externa (n=137) citada
  no artigo de derivação do escore'
theme: Calculadoras
title: 'Escore IABP-SHOCK II: Estratificação de Risco de Mortalidade no Choque Cardiogênico
  Pós-IAM'
---
...
## As seis variáveis e seus pesos
Pesos conferidos na ilustração central do artigo original (Pöss et al., JACC 2017, página 1917):

| Variável | Pontos |
|---|---:|
| Idade >73 anos | 1 |
| AVC prévio | 2 |
| Glicemia na admissão >191 mg/dL (>10,6 mmol/L) | 1 |
| Creatinina na admissão >1,5 mg/dL (>132,6 μmol/L) | 1 |
| Lactato arterial na admissão >5 mmol/L | 2 |
| Fluxo TIMI <3 após a ICP | 2 |

A soma varia de 0 a 9. Os sinais de desigualdade são estritos: um valor exatamente no limiar não satisfaz a variável. O fluxo TIMI é aferido após a ICP; o escore completo não é uma ferramenta inteiramente pré-procedimento. Ele estima prognóstico e não determina, isoladamente, indicação ou futilidade de suporte circulatório mecânico.

## Estratificação de risco e desempenho
Três categorias de risco, resultantes da soma dos pontos (máximo de 9), conferidas no artigo original:

| Categoria de risco | Pontuação | Mortalidade em 30 dias (derivação, n=480) | Mortalidade (validação externa, CardShock, n=137) |
|---|---|---|---|
| Baixo risco | 0 a 2 | 23,8% | 28,0% |
| Risco intermediário | 3 ou 4 | 49,2% | 42,9% |
| Alto risco | 5 a 9 | 76,6% | 77,3% |

- Diferença entre categorias: **p < 0,0001** na coorte de derivação; **p < 0,001** na validação externa;
- **Discriminação (AUROC/c-estatística)**: **0,74** na derivação (n=480), **0,79** na validação pelo registro IABP-SHOCK II (n=98) e **0,73** na validação externa (CardShock, n=137) — discriminação boa a moderada nas duas coortes;
...
- **Distinção do Escore SAVE**: o SAVE prediz sobrevida **pré-canulação de ECMO-VA**, num subgrupo já definido como candidato a suporte mecânico avançado; o IABP-SHOCK II Risk Score foi desenvolvido em choque cardiogênico pós-IAM tratado com ICP; não se deve extrapolá-lo automaticamente para outras etiologias ou pacientes sem revascularização.
...
- **Não substituir creatinina pela depuração calculada**, nem fluxo TIMI pós-ICP por uma medida pré-ICP: são variáveis distintas das usadas no modelo.
...
| Pontuação por variável | Idade/glicemia/creatinina: 1; AVC/lactato/TIMI: 2 |
...
| AUROC (derivação, n=480) | 0,74 |
| AUROC (validação no registro, n=98) | 0,79 |
