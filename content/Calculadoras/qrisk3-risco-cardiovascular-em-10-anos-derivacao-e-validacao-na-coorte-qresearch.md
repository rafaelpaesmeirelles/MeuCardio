...
**Sobre os coeficientes numéricos**: o artigo do BMJ descreve as variáveis do modelo e reporta as medidas de desempenho, mas **não publica a tabela completa de coeficientes de regressão** no corpo do texto principal — a implementação de referência é mantida separadamente pela ClinRisk (empresa cofundada por um dos autores). Este documento **não reproduz** nenhuma tabela de coeficientes obtida de calculadora online: conforme a diretriz de redação desta base, uma tabela de coeficientes que só aparece numa calculadora (fonte secundária) não conta como confirmada. Onde for necessário o valor exato de um coeficiente, o campo correspondente é: **VERIFICAÇÃO HUMANA NECESSÁRIA**.
