...
- 'Hippisley-Cox J, Coupland C, Brindle P. Development and validation of QRISK3 risk prediction algorithms to estimate
  ...
  PMID: 28536104. PMCID: PMC5441081 — artigo de derivação e validação, conferido nesta sessão via PubMed E-utilities
  (esearch + efetch, registro/abstract lido diretamente)'
- 'Mortensen MB, Tybjærg-Hansen A, Nordestgaard BG. Statin Eligibility for Primary Prevention of Cardiovascular
  ...
  2022;7(8):836-843. DOI: 10.1001/jamacardio.2022.1876. PMID: 35793078. PMCID: PMC9260641 — usado aqui apenas como
  validação externa de desempenho do QRISK3 fora do Reino Unido (coorte dinamarquesa), não como fonte de coeficientes'
- 'Livingstone S, Morales DR, Fleuriot J, Donnan PT, Guthrie B. External validation of the QLifetime cardiovascular
  risk prediction tool: population cohort study. BMC Cardiovasc Disord. 2023;23(1):194. DOI: 10.1186/s12872-023-03209-8.
  PMID: 37061672. PMCID: PMC10105395 — usado aqui apenas para confirmar o uso do limiar QRISK3 ≥10% na prática clínica
  britânica (CPRD) e o número necessário a tratar associado'
- 'NICE NG238 recomendações1.6.7/1.6.8: https://www.nice.org.uk/guidance/ng238/chapter/recommendations'
legacy_source: 'Documento novo, escrito em 08/09/2026. A pasta Calculadoras já cobria as três grandes famílias de
  risco cardiovascular em prevenção primária de origem americana e europeia — Framingham Risk Score (framingham-risk-score-frs),
  Pooled Cohort Equations e Equações PREVENT (pooled-cohort-equations-e-equacoes-prevent-risco-cardiovascular-em-prevencao-primaria)
  e SCORE2/SCORE2-OP, incluindo aplicabilidade brasileira e a variante SCORE2-Diabetes (score2-e-score2-op, score2-score2-op-aplicabilidade-e-limites-no-paciente-brasileiro,
  score2-diabetes-estimativa-de-risco-cardiovascular-em-10-anos-no-diabetes-tipo-2) — mas nenhum documento tratava
  do QRISK3, a ferramenta britânica (QResearch/NHS) que é hoje o modelo de risco cardiovascular com o maior número
  de variáveis clínicas incorporadas (doença renal crônica, variabilidade da pressão arterial, migrânea, corticosteroides,
  lúpus, antipsicóticos atípicos, doença mental grave e disfunção erétil), e que por isso levanta uma pergunta distinta
  das demais: o ganho de variáveis clínicas se traduz em ganho de discriminação, ou apenas em reclassificação de
  subgrupos específicos? Verificado via PubMed E-utilities nesta sessão (esearch por autor ''Hippisley-Cox QRISK3''
  e por termo ''QRISK3 external validation''; efetch de abstract nos três PMIDs acima).'
summary: null
tags: []
evidence_level: null
source_tier: A
...
**Sobre o cálculo**: esta síntese descreve o estudo, não implementa a equação nem reproduz coeficientes. Para cálculo individual, deve-se usar uma implementação autorizada e validada para a versão desejada.
...
Este é o único dado de desempenho do QRISK3 fora do Reino Unido conferido nesta pesquisa. Desempenho em população brasileira especificamente: **não avaliado pelas fontes aqui incluídas**.

## O que muda na prática — quando usar em vez de Framingham, PCE/PREVENT ou SCORE2

- O QRISK3 é a ferramenta de referência dentro do sistema de saúde britânico (NHS), citada na literatura internacional como "**UK-QRISK3**" quando comparada a outras diretrizes de prevenção primária (PMID 35793078). Livingstone S et al., *BMC Cardiovasc Disord*. 2023;23(1):194 (PMID 37061672), usam o limiar **QRISK3 ≥ 10%** de risco em 10 anos como corte prático para estatina em população do Clinical Practice Research Datalink (CPRD, Reino Unido), estimando número necessário a tratar (NNT) em 10 anos de **34 em mulheres e 37 em homens** nesse limiar. A recomendação 1.6.7 do [NICE NG238](https://www.nice.org.uk/guidance/ng238/chapter/recommendations) oferece atorvastatina 20 mg para prevenção primária quando QRISK3 em dez anos é ≥10%, após discussão clínica. Risco inferior a 10% não exclui tratamento diante de preferência informada ou suspeita de subestimação (1.6.8).
...
- A vantagem clínica real do QRISK3 sobre as demais famílias é capturar **risco adicional em pacientes com comorbidades específicas** que não aparecem todas, explicitamente, em outras equações: doença renal crônica estágio 3 (não só 4-5), lúpus, uso crônico de corticosteroide, antipsicótico atípico, doença mental grave, migrânea e disfunção erétil (homens). A ausência de uma variável explícita não prova subestimação individual; PREVENT, por exemplo, incorpora função renal. A comparação exige validação de calibração para população e desfecho equivalentes.
...
- **Assumir que HIV/AIDS é variável de entrada do QRISK3** porque aparece na lista de "novos fatores testados" — não é: o próprio artigo de derivação relata que HIV/AIDS **não atingiu significância estatística** e foi excluído do modelo final. A disfunção erétil (em homens) foi incorporada, junto com DRC estágio 3, variabilidade da PAS, migrânea, corticosteroide, LES, antipsicótico atípico e doença mental grave.
