...
O próprio artigo descreve o desempenho geral do QRISK3 atualizado como **semelhante** ao do QRISK2 — ou seja, os novos preditores melhoram a granularidade clínica (identificar subgrupos específicos de risco elevado, como pacientes com LES ou doença mental grave) sem necessariamente elevar de forma expressiva a discriminação agregada em toda a população. Em subgrupos analisados no texto completo, o desempenho variou por faixa etária (mais alto no grupo mais jovem, de 25-39 anos, e mais baixo no grupo mais idoso, de 60-84 anos) e foi visivelmente mais fraco em diabetes tipo 2 do que na população geral — **VERIFICAÇÃO HUMANA NECESSÁRIA** quanto ao valor exato da estatística C nesse subgrupo específico, por ter sido extraído de leitura assistida do texto completo e não conferido diretamente linha a linha nesta sessão.
...
- **VERIFICAÇÃO HUMANA NECESSÁRIA** quanto à situação de uso do QRISK3 fora do Reino Unido, à eventual existência de recalibração específica para outra população (nenhuma foi confirmada nas fontes consultadas nesta sessão) e à comparação direta entre QRISK3 e QRISK2 no texto de origem além do que já foi citado acima
