...
review_note: "Produção científica assistida (Claude, 26/08/2026); pendente de revisão independente e validação médica final. Texto integral da diretriz ESC 2024 (Eur Heart J 2024;45:3912-4018) lido nesta sessão via cópia do Wayback Machine da página da Oxford Academic — seções 7.5 (definição de hipertensão resistente, Tabela 10 e Tabela 11), 8.3.2 (classes e betabloqueador), 8.3.4 (combinações e titulação), 8.3.5 (algoritmo prático e legenda da Figura 18), 8.4, 8.5.1, 8.5.4, 8.7.2, 9.11 e 9.13.3; a Tabela de Recomendação 16 é imagem e não pôde ser lida, por isso as classes de recomendação foram tomadas do comentário dos próprios autores da diretriz (McCarthy et al., EHJ-CVP 2025, PMC11805683, lido na íntegra). Derivado dos documentos já publicados hipertensao-arterial-e-pressao-arterial-elevada-esc-2024.md e hipertensao-arterial-classificacao-diagnostico-metas.md, e coordenado com os fluxogramas existentes da ESC 2024 (classificação e alvo) e da hipertensão resistente (quarta droga), que não foram repetidos. Verificação adversarial (Claude, 26/08/2026), com o texto integral da diretriz reaberto: DOIs e PMIDs confirmados no Crossref e no PubMed; corrigida a afirmação de que a ESC 2024 prefere IECA + BCC a IECA + tiazídico (a diretriz apresenta as duas duplas como equivalentes; o ACCOMPLISH é evidência de desfecho, não recomendação da diretriz); acrescentado o ramo de intolerância a um componente nos passos 1 e 2 (troca de classe no mesmo passo, sem avançar), que a árvore original não previa."
source_refs:
...
- A Tabela de Recomendação 16 da diretriz, que reúne as classes e níveis de evidência do tratamento farmacológico, é publicada como imagem e não pôde ser lida; as classes citadas aqui — Classe I para a combinação dupla inicial em dose baixa, para as quatro classes de primeira linha e para o betabloqueador com indicação específica, Classe IIa para a espironolactona — vêm do texto corrido da diretriz e do comentário dos próprios autores em EHJ-CVP. Os níveis de evidência (A, B, C) não foram confirmados — VERIFICAÇÃO HUMANA NECESSÁRIA.
- As doses de cada fármaco (Tabelas Suplementares S7 e S8) não foram acessadas; o fluxograma trabalha só com "dose baixa" e "dose máxima tolerada", como faz o texto principal — VERIFICAÇÃO HUMANA NECESSÁRIA antes de acrescentar valores.
