...
kind: fluxograma
summary: "No ambulatório, miocardite por ICI começa como dor torácica atípica, dispneia, fadiga, palpitação ou CK/AST 'estranhos'. Este fluxograma separa o que pode aguardar eco/troponina no mesmo dia do que exige PS/UTI — complemento ambulatorial ao fluxograma de emergência já existente."
...
- sinais de IC nova;
- mialgia intensa, fraqueza proximal, ptose, diplopia ou dispneia neuromuscular — pensar overlap **miocardite–miosite–miastenia**;
...
flowchart TD
  ...
  B -->|Não| C["Mesmo dia ambulatorial ou PS se sem acesso rápido:<br/>ECG + troponina hs + BNP/NT-proBNP<br/>+ CK ± enzimas<br/>Avaliar força muscular / sintomas miastênicos"]

  C --> D{"ECG anormal OU<br/>troponina elevada OU<br/>sinais de IC OU<br/>suspeita de overlap
miosite/miastenia"}

  D -->|Sim| E["Suspender ICI<br/>Eco urgente<br/>Contatar cardio-oncologia<br/>Considerar RM cardíaca / internar<br/>conforme gravidade"]
  ...
  F -->|Sim| G["Repetir troponina/ECG em curto intervalo<br/>Eco preferencial<br/>Não liberar próximo ciclo de ICI<br/>sem reassessment"]

  F -->|Não — quadro banal<br/>e workup negativo| H["Registrar baseline<br/>Educar red flags<br/>Liberar retorno oncológico<br/>com aviso explícito à equipe"]

  E --> I{"Há fraqueza,<br/>ptose, diplopia<br/>ou CK muito alta?"}
  I -->|Sim| J["Ativar overlap miocardite-miosite-miastenia<br/>Evitar fármacos que agravem bloqueio neuromuscular<br/>Avaliação neurológica urgente"]
