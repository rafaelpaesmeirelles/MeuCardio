...
review_note: "Revisado em 26/08/2026 contra a seção 6.5, a definição de toxicidade vascular e a Tabela de Recomendação 33 da diretriz ESC 2022 de cardio-oncologia (PMID 36017568). Corrigidos os operadores dos limiares: pausar a terapia causal em PAS >=180 mmHg ou PAD >=110 mmHg; recomendar combinação IECA/BRA + bloqueador de canal de cálcio di-hidropiridínico quando PAS >=160 mmHg e PAD >=100 mmHg. O reinício ficou condicionado a PAS <160 e PAD <100 mmHg, com consideração de redução de dose. Lesão aguda de órgão-alvo, e não o número isolado, continua definindo emergência hipertensiva. Pendente revisão médica independente antes de uso assistencial."
...
flowchart TD
  ...
  P4["Controle rápido: iniciar/otimizar IECA ou BRA<br/>associado a bloqueador de canal de cálcio<br/>di-hidropiridínico, salvo contraindicação"]
  ...
  C1(["PA abaixo dos limiares graves:<br/>tratar conforme risco cardiovascular e contexto<br/>oncológico + monitorização seriada/domiciliar"])
  C2(["Após PAS <160 E PAD <100 mmHg:<br/>MDT decide reinício, considerando redução de dose<br/>ou terapia oncológica alternativa"])
...
O limiar de **PAS ≥160 e PAD ≥100 mmHg exige que os dois componentes estejam
presentes** para a recomendação específica de combinação imediata da ESC. Isso
não significa ignorar elevação isolada de um componente: abaixo do limiar de pausa,
ela ainda requer tratamento conforme risco cardiovascular, causas reversíveis e
contexto oncológico.
