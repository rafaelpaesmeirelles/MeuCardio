...
legacy_source: Documento novo, escrito em 09/09/2026. A pasta já tem um documento
  ...
  sueca de coirmãs de Crump/Sundquist (Eur Heart J 2025, PMID 40569854) e vários documentos
  sobre risco cardiovascular agregado ou sobre subtipos de distúrbio hipertensivo.
  Nenhum documento desta pasta isolava a doença arterial periférica (DAP) como desfecho
  vascular próprio — leito vascular distinto do coronariano e do cerebrovascular,
  historicamente sub-estudado, e que este novo artigo de 2026 do mesmo grupo (mesma
  base de dados sueca, desenho de coirmãs) trata com o mesmo rigor metodológico já
  usado para AVC nesta pasta. A busca em PubMed E-utilities (esearch) confirmou que
  não há artigo equivalente já coberto aqui, e o texto integral foi lido diretamente
  da fonte primária (PMC, acesso aberto) nesta sessão, não de resumo.
published: true
review_note: Texto completo PLoS e tabelas 2/3 conferidos; corrigidos denominador
  dos casos, mediana de seguimento, maior fração atribuível (PIG), janela dos múltiplos
  antecedentes e causalidade/HR temporal.
review_status: revisado
slug: doenca-arterial-periferica-apos-desfechos-gestacionais-adversos-coorte-sueca-de-coirmas-2026
source_refs:
- 'Crump C, Wei J, Sundquist J, Sundquist K. Adverse pregnancy outcomes and long-term
  risk of peripheral artery disease: a cohort study. PLoS Med. 2026;23(7):e1004821.
  DOI: 10.1371/journal.pmed.1004821. PMID: 42479723 — texto integral em acesso aberto
  (CC BY 4.0), PMCID PMC13387525, lido na íntegra nesta sessão via XML completo do
  PubMed Central (eutils efetch, db=pmc); coorte nacional sueca de nascimentos únicos
  1973-2015, 2.201.446 mulheres, 54 milhões de pessoas-ano de seguimento até 2018,
  análise de coirmãs para confundimento familiar'
theme: Gravidez
title: 'Doença Arterial Periférica Após Desfechos Gestacionais Adversos: Coorte Sueca
  de Coirmãs (2026)'
---
...
| Desfecho gestacional | HR não estratificado | HR estratificado por irmãs |
|---|---|---|
| Diabetes gestacional | 2,62 (2,17-3,16) | **2,97** (1,93-4,57) |
| Parto pré-termo | 1,70 (1,58-1,83) | **1,62** (1,40-1,89) |
| Pequeno para idade gestacional | 1,70 (1,60-1,80) | **1,56** (1,39-1,75) |
...
Quatro dos cinco desfechos **mantiveram associação significativa** mesmo comparando irmãs entre si — o achado não é explicado, na maior parte, por predisposição familiar compartilhada. A associação com diabetes gestacional, em particular, **não se atenuou** na análise de coirmãs (foi 2,97 no modelo estratificado e 2,62 no não estratificado da mesma subcoorte), sem excluir confundimento residual ou estabelecer causalidade.
...
- **Parto pré-termo por indicação médica** associou-se a DAP (HR ajustado 2,16; IC95% 1,82-2,56), mas **a associação com parto pré-termo espontâneo não atingiu significância estatística** (HR 1,23; IC95% 0,99-1,53; p=0,06) — distinção clinicamente relevante que o desfecho agregado "parto pré-termo" esconde.
- Mulheres com **múltiplos desfechos gestacionais adversos** tiveram HRs maiores de DAP na janela de 30–46 anos após o parto: 1 desfecho, HR 1,40 (1,34-1,47); 2 desfechos, HR 1,83 (1,69-1,98); ≥3 desfechos, HR 2,77 (2,32-3,30) — tendência linear significativa (p<0,001).

## Síntese prática

Os cinco antecedentes obstétricos tiveram associação ajustada com DAP no seguimento completo. A história gestacional deve ser considerada na avaliação vascular, mas os HRs deste estudo não definem um escore individual nem o efeito de uma intervenção preventiva.

O excesso absoluto de incidência cresceu com o envelhecimento; os riscos relativos não aumentaram uniformemente. Por exemplo, o HR de parto pré-termo foi 2,02 antes de dez anos e 1,58 entre 30 e 46 anos. As associações persistiram após ajuste entre coirmãs para quatro antecedentes, mas essa análise reduz apenas parte do confundimento familiar e não comprova causalidade.

## Armadilhas clínicas
- **Não perguntar sobre desfecho gestacional adverso ao investigar claudicação ou DAP em mulher de meia-idade ou mais velha** — a associação pode persistir até 46 anos após o parto, uma janela de tempo que ultrapassa a memória clínica habitual da anamnese obstétrica.
- **Tratar bebê PIG como desfecho exclusivamente neonatal** — ele carrega HR de 1,72-1,74 para DAP materna a longo prazo, e teve a maior fração atribuível populacional estimada no seguimento completo (11,8%, contra 9,2% para parto pré-termo; essas estimativas dependem de pressupostos causais).
...
- **Extrapolar as taxas absolutas suecas (0,6% de incidência de DAP na coorte) diretamente para a população brasileira** sem ajuste — a coorte é sueca, de base populacional e seguimento por registro nacional; tanto as taxas quanto a magnitude das associações precisam de avaliação de validade externa.
...
- **Registro ambulatorial especializado sueco só tem cobertura nacional a partir de 2001** — diagnósticos de DAP em anos anteriores tendem a estar sub-registrados, o que pode subestimar o risco absoluto real associado a cada desfecho gestacional.
- IMC e tabagismo só foram aferidos no início do pré-natal (não em momentos posteriores da vida), e história prévia de hipertensão/diabetes/dislipidemia veio de diagnósticos administrativos, sujeitos a subnotificação.
- Estudo limitado à Suécia — os próprios autores registram a necessidade de replicação em outras populações, inclusive para avaliar diferenças raciais/étnicas não capturadas nesta coorte.
- Coorte ainda relativamente jovem ao fim do seguimento (idade mediana 53 anos) — o risco absoluto de DAP tende a ficar mais evidente com o envelhecimento da coorte além do que este seguimento já capturou.
