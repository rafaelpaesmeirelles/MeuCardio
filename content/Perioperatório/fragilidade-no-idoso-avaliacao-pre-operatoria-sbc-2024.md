---
title: "Fragilidade no idoso: avaliação pré-operatória segundo a SBC 2024"
slug: fragilidade-no-idoso-avaliacao-pre-operatoria-sbc-2024
...
```mermaid
flowchart TD
    A["Paciente idoso candidato a cirurgia não cardíaca"] --> B{"Cirurgia de risco intermediário ou alto?"}
    B -->|"Não"| C["Avaliar fragilidade quando clinicamente relevante;<br/>não presumir fragilidade apenas pela idade"]
    B -->|"Sim"| D["Aplicar instrumento objetivo validado"]
    D --> E{"Resultado"}
    E -->|"Robusto"| F["Prosseguir integração com risco cardiovascular e capacidade funcional"]
    E -->|"Pré-frágil"| G["Identificar déficits modificáveis e planejar prevenção de complicações"]
    E -->|"Frágil"| H["Discussão multidisciplinar de risco-benefício + plano perioperatório intensificado"]
    G --> I["Considerar pré-habilitação física/nutricional,<br/>correção de anemia e prevenção de delirium conforme déficits"]
    H --> I
    I --> J["Planejar recuperação funcional, destino pós-alta e nível de monitorização"]
```
