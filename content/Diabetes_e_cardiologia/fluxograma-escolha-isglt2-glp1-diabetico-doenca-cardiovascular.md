...
flowchart TD
  ...
  P1 --> D4{"Albuminúria persiste<br/>após otimização do tratamento?"}

  D4 -->|"Sim"| C3(["Associar finerenona ao inibidor<br/>de SGLT2 já em uso"])
...
- **A combinação das duas classes não é um ramo isolado da árvore, porque ela é
  ...
  et al. (CMAJ 2026, 99.683 participantes) mostrou que associar iSGLT2 e GLP-1RA reduz
  hospitalização por insuficiência cardíaca de forma estatisticamente significativa frente
  a cada monoterapia isolada (RR 0,72 frente ao iSGLT2 isolado; RR 0,62 frente ao GLP-1RA
  isolado), sem prova estatística de redução adicional de eventos cardiovasculares maiores
  (MACE) ou de eventos renais maiores neste corte de dados — a decisão de associar
  continua sendo sustentada primeiro pelo benefício já comprovado de cada classe isolada
  contra placebo, com a hospitalização por IC como o desfecho onde a combinação hoje tem a
  evidência mais forte de superioridade.
