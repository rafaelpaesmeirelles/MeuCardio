---
title: "Agonistas de Incretina na ICFEp com Obesidade: STEP-HFpEF versus SUMMIT"
...
```mermaid
flowchart TD
  A["IC sintomática + evidência objetiva de ICFEp + IMC ≥ 30 kg/m²"] --> B{"Congestão atual?"}
  B -->|Sim| C["Diurético até euvolemia — trata sintoma, não substitui o restante"]
  B -->|Não| D["FMT da ICFEp: iSGLT2 se não houver contraindicação; MRA conforme ESC 2026 / FINEARTS-HF"]
  C --> D
  D --> E{"FEVE ≥ 45% e IMC ≥ 30, independente de diabetes?"}
  E -->|Não| F["Não usar STEP/SUMMIT como indicação: ensaio e diretriz pedem obesidade; ICFEr fica de fora"]
  E -->|Sim| G["Considerar semaglutida 2,4 mg ou tirzepatida até 15 mg semanal — ESC IIa B1 para peso, exercício e QoL"]
  G --> H{"O que se pode prometer"}
  H --> I["Semaglutida / STEP: sintomas, limitação física, exercício e peso em 52 semanas"]
  H --> J["Tirzepatida / SUMMIT: composto de morte CV ou piora da IC (puxado pela piora) + KCCQ; NÃO mortalidade isolada"]
  G --> K["Consentir náusea/vômito/diarreia e risco de descontinuação; não retirar iSGLT2"]
```
