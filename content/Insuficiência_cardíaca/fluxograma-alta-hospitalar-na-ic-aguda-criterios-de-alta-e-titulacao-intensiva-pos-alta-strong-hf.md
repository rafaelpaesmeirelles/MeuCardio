...
review_note: "Produção científica assistida (Claude, 26/08/2026); pendente de revisão independente e validação médica final. Lidos na íntegra nesta sessão: texto da atualização focada ESC 2023 em academic.oup.com (descrição do protocolo do STRONG-HF, desfecho primário, doses atingidas, eventos adversos graves e lista de parâmetros a vigiar nas visitas; a tabela de recomendação é imagem e a classe I nível B foi confirmada pelo resumo da ACC e pelo documento do acervo já verificado); resumo indexado do STRONG-HF (PubMed 36356631, via E-utilities); resumo da análise de indicadores de segurança do STRONG-HF (Tomasoni 2024, PubMed 37820896) e do artigo de desenho do ensaio (Kimmoun 2019, PubMed 31423712); pontos-chave da ACC sobre a ESC 2021 (recomendação classe I de excluir congestão antes da alta, testar terapia oral e consulta em 1 a 2 semanas). Texto integral do Lancet e da análise de segurança bloqueados. Derivado dos documentos do acervo safety-tolerability-and-efficacy-of-up-titration-of-guideline-directed-medical-therapies-for-acute-heart-failure-strong-hf e atualizacao-focada-2023-das-diretrizes-esc-2021-de-insuficiencia-cardiaca. A regra por fármaco diante de cada indicador de segurança vem de fonte secundária e está marcada para verificação humana. Verificação adversarial (26/08/2026): conferidos DOIs no Crossref, PMIDs no PubMed, resumos do Lancet, J Card Fail e EJHF via E-utilities, texto da ESC 2023 e seção 11.3.11 da ESC 2021 em academic.oup.com, pontos-chave da ACC e registro NCT03412201; corrigidos o NNT (8,1% de diferença absoluta corresponde a cerca de 1 em 12, não 1 em 8), a afirmação de que não havia critério de exclusão por idade (o ensaio excluiu acima de 85 anos, TFGe abaixo de 30 e incapacidade de seguimento), acrescentado o nível C das recomendações pré-alta da ESC 2021 e as associações isoladas de TFGe abaixo de 30 e PAS abaixo de 95 da análise de Tomasoni; quelante restrito à hipercalemia na conduta C7."
source_refs:
...
A atribuição de cada indicador a fármacos específicos, tal como aparece nas condutas C5 a C8, foi obtida de resumo secundário do protocolo, e não do texto integral do Lancet, que ficou inacessível: VERIFICAÇÃO HUMANA NECESSÁRIA. O que é primário e está confirmado são os cinco limiares e a regra de adiar a titulação. Pausar não é retirar: a dose atual se mantém, o laboratório se repete na visita seguinte e a titulação recomeça quando o indicador some. Hipotensão sintomática que persiste tem fluxograma próprio nesta pasta; hipercalemia que impede o bloqueio do SRAA e do ARM tem no fluxograma de sequenciamento a opção de quelante antes de reduzir dose.
...
- **Regra por fármaco diante de cada indicador de segurança** (condutas C5 a C8) vem de resumo secundário do protocolo; os limiares numéricos são primários, a atribuição por classe farmacológica precisa ser conferida no apêndice do Lancet: VERIFICAÇÃO HUMANA NECESSÁRIA.
