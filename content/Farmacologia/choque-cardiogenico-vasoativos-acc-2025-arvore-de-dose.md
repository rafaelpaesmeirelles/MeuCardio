...
```mermaid
flowchart TD
    A[Choque cardiogênico] --> B[Avaliar PA/MAP, débito, congestão, perfusão e SVR]
    B --> C{Hipotensão relevante?}
    C -->|Sim| D[Norepinefrina é escolha inicial razoável para a maioria]
    D --> E[0,05-1 mcg/kg/min]
    C -->|Não / normotenso| F{Baixo débito + SVR elevada?}
    F -->|Sim| G[Considerar inodilatador ou vasodilatador conforme hemodinâmica]
    G --> H[Dobutamina 2-10 mcg/kg/min]
    G --> I[Milrinona 0,125-0,5 mcg/kg/min]
    G --> J[Nitroprussiato 0,3-10 mcg/kg/min]

    B --> K{Bradicardia é componente importante?}
    K -->|Sim| L[Considerar cronotrópico conforme contexto]
    L --> M[Isoproterenol 2-20 mcg/min]

    B --> N{Agente escolhido = fenilefrina isolada?}
    N -->|Sim| O[Evitar como única infusão de 1ª linha no choque cardiogênico]

    I --> P{Função renal em piora?}
    P -->|Sim| Q[Usar milrinona com cautela: eliminação renal e meia-vida longa]
```
